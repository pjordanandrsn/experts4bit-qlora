...
- **Review fixes (acting CTO, 2026-09-06, PR #440 review):** the launcher refreshes the guard's heartbeat for the whole `--command` (a guard-initiated teardown -- `heartbeat-loss` / `wallclock` -- is recorded as `status ALARM`, `result invalid`, never `pass`); `docs/compute-policy.json` gains `approval_overrides` (while the CTO seat is held by `cursor-desktop-mini/grok`, $2-$20 requires `all-of:CTO,CSO` -- Jordan's ruling) honoured by the launcher (`--seat-executor ROLE=EXECUTOR`, recorded in `environment.seat_executors`) and by `scripts/check_run_ledger.py`; `commit_sha` / `branch` / `dirty_tree` come from git and every receipt is validated against `docs/run-receipt-schema.json` before it is written; `--role`, `--agent`, `--work-id`, `--preregistration`, `--hypothesis`, `--expected-result`, `--success-criteria`, `--failure-criteria` are required (no identity defaults; `decision` is `pending review`; `cost_usd.actual` is the provider's billing, never a copy of the estimate); a live-provider refusal writes a REFUSED receipt; a role without a numeric ceiling row is refused (also in the ledger check); approval permalinks must be `https://cerin-amroth.slack.com/archives/C…/p<16 digits>` and Jordan's approval needs role and agent `Jordan`; the guard exits once the instance is gone. **Round 2 (Warden source read of 7fda840):** an undeclared seat is never the loophole -- the override applies unless every seat it names is declared and held by someone else, in the launcher and the ledger check; `pass` only when the launcher itself destroyed a live instance (an instance already gone, with or without a guard proof, is `ALARM`/`invalid`); a failing `destroy` is recorded, not raised; proofs are written atomically and a guard proof never overwrites the launcher's; the no-jsonschema fallback checks approvals, `teardown_proof`, timestamps, artifacts and numeric types, and a missing schema file refuses; the ledger check's Jordan test and permalink pattern match the launcher's; `--cpu/--ram/--storage`; `decision` is `pending <work-id>`. **Round 3 (one CI failure on 165ecb9):** the launcher waits for the guard's arm marker (`guard-armed.json`, `--guard-arm-timeout-s`, default 60 s) before it runs anything -- the guard is a `python -m` process that imports this package and starts seconds late on a cold box, so the old launcher ran `--command` in an unguarded window (and the stalled-heartbeat test raced that window); a guard that never arms means teardown without the command (`status HARNESS_ERROR`, `result invalid`, `reason guard-not-armed`); the guard's clock starts at arming (fresh heartbeat, then the marker); `environment.guard_armed_at` is recorded. The strict validator (`jsonschema`, now in the `test` extras so CI runs it) rejected what the fallback had let through: `environment` values must be strings (`seat_executors` is a JSON string; the ledger check parses it) and `gpu_count >= 1` (a refusal records the requested count with the `not-launched` proof); the fallback checks both now.
...
- **Review fixes (acting CTO, 2026-09-06)**: the manifest's identity fields (`schema_version`, `layout`, `model_id`, `model_revision`, `layers[]`) are written a second time as a hashed payload `payloads/identity.json`, so the root `pack_fingerprint` covers which checkpoint and layout the bytes belong to; `verify_artifact` refuses a manifest whose top-level copy disagrees with the hashed one. The licensed loader takes N/K from the hashed payload bytes (`packed [E, N, K//2] uint8`, `scales [E, N, K//32]`) and treats `layers[]` only as a cross-check that refuses on disagreement. `dump_calibrated_artifact` refuses an unknown `model_revision` unless `allow_unknown_revision=True` (then the pack carries `model_revision_missing: true`).
