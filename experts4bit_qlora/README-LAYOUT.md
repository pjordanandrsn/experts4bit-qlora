...
| `engines/` | execution paths and residency policy (`pipelined`, `cold_engine`, `hot_residency`, `offload`, `dense_offload`, `nvme_experts`, `fast`, `batched`, `capture`, `kv_cache`, `speculative`, `expert_profile`) | you are changing **how compute or memory is scheduled**, not what is read |
