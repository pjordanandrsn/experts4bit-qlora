...
> **Staged, not wired.** Having a convention and being loadable are different questions. **9 model_types across 7 conventions have a record here that no loader path admits** — `axk1`, `granitemoehybrid`, `granitemoeshared`, `qwen3_vl_moe`, `qwen3_vl_moe_text`, `jamba`, `lfm2_moe`, `jetmoe`, `dbrx`. `moe_conventions.STAGED_NOT_WIRED` is the authority and is asserted against what the loader actually refuses, in both directions. Both counts above are asserted against the set, so neither can drift. (`axk2`, by contrast, has no `SUPPORTED_ARCHITECTURES` entry either but aliases onto `qwen2_moe` and **is** admitted.) `nemotron_h` was on this list and is now **wired** (#648) — admission plus the prefix rename its expert path needed, with a real-checkpoint row. Refs #648, #509.
