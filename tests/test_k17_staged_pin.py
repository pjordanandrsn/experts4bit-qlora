"""The k17 lane's staged-file pin must match the repo (the e4b#642 check, mirrored for K17).

`bench/k17/k17_drive.sh` refuses to run when a staged file's sha256 differs from `bench/k17/staged.sha256`;
that guard runs on the CONTROLLER after a box is rented. This test runs the same comparison in CI, where it
costs nothing. It mirrors the driver's own staging rule: the only staged piece is the lane's runner (the
harness is cloned ON THE BOX from grouped-nf4-gemm at GNF4_SHA), so every name resolves to `bench/k17/`.
"""
import hashlib
import pathlib
...
@pytest.mark.skipif(not PIN.exists(), reason="k17 lane not present")
...
def test_staged_file_matches_its_pin(want, name):
    ...
    assert got == want, (
        ...
        f"Re-pin it, or the next k17 launch refuses ON A RENTED BOX."
    )
...
def test_every_staged_piece_the_driver_names_is_pinned():
    ...
    driver = (LANE / "k17_drive.sh").read_text()
