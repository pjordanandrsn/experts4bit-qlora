"""The p58 lane's staged-file pin must match the repo (the e4b#642 check, mirrored for P58 (the same-box vLLM head-to-head re-run)).

`bench/p58/p58_drive.sh` refuses to run when a staged file's sha256 differs
...
PIN = REPO / "bench" / "p58" / "staged.sha256"


def _resolve(name: str) -> pathlib.Path:
    """p58_drive.sh's staging `case`, verbatim: the runner and the reducer from bench/p58; p37_vllm.py from
    bench/h2h-20260905/p37 (byte-identical to P37's); the hook from bench/p42; everything else
    (step_decomp.py, k8_bake.py, calib.json) from bench/p39."""
    if name in ("p58_run.sh", "p58_reduce.py"):
        return REPO / "bench" / "p58" / name
    if name == "p37_vllm.py":
        return REPO / "bench" / "h2h-20260905" / "p37" / name
...
@pytest.mark.skipif(not PIN.exists(), reason="p58 lane not present")
...
def test_staged_file_matches_its_pin(want, name):
    ...
    assert got == want, (
        ...
        f"Re-pin it, or the next p58 launch refuses ON A RENTED BOX."
    )
...
def test_every_staged_piece_the_driver_names_is_pinned():
    ...
    driver = (REPO / "bench" / "p58" / "p58_drive.sh").read_text()
    ...
    named.add("p58_run.sh")                      # $HERE/p58_run.sh, the lane's own runner
