"""The p60 lane's staged-file pin must match the repo (the e4b#642 check, mirrored for P60 (the same-box vLLM head-to-head re-run)).

`bench/p60/p60_drive.sh` refuses to run when a staged file's sha256 differs
...
PIN = REPO / "bench" / "p60" / "staged.sha256"


def _resolve(name: str) -> pathlib.Path:
    """p60_drive.sh's staging `case`, verbatim: the runner, recorder, replay bench and reducer from bench/p60;
    serve_stack.py from bench/p44; the hook from bench/p42; everything else (step_decomp.py, k8_bake.py, calib.json)
    from bench/p39."""
    if name in ("p60_run.sh", "record_eids.py", "replay_gemv.py", "p60_reduce.py"):
        return REPO / "bench" / "p60" / name
...
@pytest.mark.skipif(not PIN.exists(), reason="p60 lane not present")
...
def test_staged_file_matches_its_pin(want, name):
    ...
    assert got == want, (
        ...
        f"Re-pin it, or the next p60 launch refuses ON A RENTED BOX."
    )
...
def test_every_staged_piece_the_driver_names_is_pinned():
    ...
    driver = (REPO / "bench" / "p60" / "p60_drive.sh").read_text()
    ...
    named.add("p60_run.sh")                      # $HERE/p60_run.sh, the lane's own runner
