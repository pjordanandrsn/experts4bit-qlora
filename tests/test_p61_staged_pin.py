"""The p61 lane's staged-file pin must match the repo (the e4b#642 check, mirrored for P61).

`bench/p61/p61_drive.sh` refuses to run when a staged file's sha256 differs from `bench/p61/staged.sha256`;
that guard runs on the CONTROLLER after a box is rented. This test runs the same comparison in CI, where it
costs nothing. It mirrors the driver's own staging rule: the lane's runner resolves to `bench/p61/` and P60's recorded
routing (`eids_b16.int16.{bin,json}`) to `bench/p60/receipts/`. Unlike K18, the bench itself (`sweep_gemv.py`) is
staged from here and pinned too; only the served GEMV comes from grouped-nf4-gemm at GNF4_SHA.
"""
import hashlib
import pathlib
...
@pytest.mark.skipif(not PIN.exists(), reason="p61 lane not present")
...
def test_staged_file_matches_its_pin(want, name):
    ...
    assert got == want, (
        ...
        f"Re-pin it, or the next p61 launch refuses ON A RENTED BOX."
    )
...
def test_every_staged_piece_the_driver_names_is_pinned():
    ...
    driver = (LANE / "p61_drive.sh").read_text()
