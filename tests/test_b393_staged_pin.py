"""The b393 lane's staged-file pin must match the repo (the e4b#642 check, mirrored for B393).

`bench/b393/b393_drive.sh` refuses to run when a staged file's sha256 differs from `bench/b393/staged.sha256`;
that guard runs on the CONTROLLER after a box is rented. This test runs the same comparison in CI, where it
costs nothing. It mirrors the driver's own staging rule: the lane's runner resolves to `bench/b393/`; the census script is
...
@pytest.mark.skipif(not PIN.exists(), reason="b393 lane not present")
...
def test_staged_file_matches_its_pin(want, name):
    ...
    assert got == want, (
        ...
        f"Re-pin it, or the next b393 launch refuses ON A RENTED BOX."
    )
...
def test_every_staged_piece_the_driver_names_is_pinned():
    ...
    driver = (LANE / "b393_drive.sh").read_text()
