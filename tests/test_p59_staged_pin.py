"""The p59 lane's staged-file pin must match the repo (the e4b#642 check, mirrored for P59 (the same-box vLLM head-to-head re-run)).

`bench/p59/p59_drive.sh` refuses to run when a staged file's sha256 differs
...
PIN = REPO / "bench" / "p59" / "staged.sha256"


def _resolve(name: str) -> pathlib.Path:
    """p59_drive.sh's staging `case`, verbatim: the runner, the scorer and the reducer from bench/p59; serve_stack.py from
    bench/p44; kl_fidelity.py from bench/; the hook from bench/p42; everything else (step_decomp.py, k8_bake.py,
    calib.json) from bench/p39."""
    if name in ("p59_run.sh", "kl_b16.py", "p59_reduce.py"):
...
@pytest.mark.skipif(not PIN.exists(), reason="p59 lane not present")
...
def test_staged_file_matches_its_pin(want, name):
    ...
    assert got == want, (
        ...
        f"Re-pin it, or the next p59 launch refuses ON A RENTED BOX."
    )
...
def test_every_staged_piece_the_driver_names_is_pinned():
    ...
    driver = (REPO / "bench" / "p59" / "p59_drive.sh").read_text()
    ...
    named.add("p59_run.sh")                      # $HERE/p59_run.sh, the lane's own runner
