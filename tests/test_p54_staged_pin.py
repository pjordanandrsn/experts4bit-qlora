"""The p54 lane's staged-file pin must match the repo (the e4b#642 check, mirrored for P54).

`bench/p54/p54_drive.sh` refuses to run when a staged file's sha256 differs
...
PIN = REPO / "bench" / "p54" / "staged.sha256"


def _resolve(name: str) -> pathlib.Path:
    """p54_drive.sh's staging `case`, verbatim: the lane's own runner from bench/p54, the hook from bench/p42,
    everything else (step_decomp.py, k8_bake.py, calib.json) from bench/p39."""
    if name.startswith("hook/"):
        return REPO / "bench" / "p42" / name
    if name == "p54_run.sh":
        return REPO / "bench" / "p54" / name
...
@pytest.mark.skipif(not PIN.exists(), reason="p54 lane not present")
...
def test_staged_file_matches_its_pin(want, name):
    ...
    assert got == want, (
        ...
        f"Re-pin it, or the next p54 launch refuses ON A RENTED BOX."
    )
...
def test_every_staged_piece_the_driver_names_is_pinned():
    ...
    driver = (REPO / "bench" / "p54" / "p54_drive.sh").read_text()
    ...
    named.add("p54_run.sh")                      # $HERE/p54_run.sh, the lane's own runner
