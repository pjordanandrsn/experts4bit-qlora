...
- **It trains.** QLoRA on the frozen NF4 experts improves a held-out Alpaca eval from
  **1.4813 → 1.0290** (see [`docs/METHODOLOGY.md`](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/METHODOLOGY.md)).
...
- **It dials.** Spare VRAM converts to decode speed continuously — the pipelined engine keeps K
  ...
  (receipts: [`bench/RESULTS-informed-hotsets.md`](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/bench/RESULTS-informed-hotsets.md)).
...
not your model. Reasoning, caveats and requirements for each: **[docs/CHOOSING.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/CHOOSING.md)**.
...
**"Does not fit host RAM" is measured, not a slogan**, and **the saving grows with the
model**. Four steps, one box, the cap descended until each arm stops completing
([receipts](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/bench/host-ram-ceiling/RESULTS-scaling.md)):

| | expert bytes | host-RAM path | arena path | |
|---|---|---|---|---|
| OLMoE-1B-7B | 3.62 GB | 5.91–6.17 GB | 2.28–2.42 GB | **2.56×** |
| Gemma-4-26B-A4B | 12.85 GB | 19.33–20.40 GB | 5.10–5.37 GB | **3.80×** |
| Qwen3-30B-A3B | 16.31 GB | 24.70–25.77 GB | 3.89–4.03 GB | **6.40×** |

At the cap in between — 8.59 GB — Qwen3-30B-A3B is **OOM-killed** on the host-RAM path and
**trains to completion** on the arena. The host side tracks total expert bytes; the arena
...
([receipts](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/bench/host-ram-ceiling/RESULTS-timing.md)):
...
([receipts](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/bench/dgrad-gate/RESULTS-dgrad-gate.md)):
...
> [fused-train gate](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/bench/fused-train-gate/RESULTS-fused-train-gate.md)
> registered.
...
Engines, selection and the host-regime laws: **[docs/RESIDENCY-ENGINES.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/RESIDENCY-ENGINES.md)**.
...
[support matrix](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/support_matrix.md).
...
Moved to **[docs/STORAGE-MODES.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/STORAGE-MODES.md)** — The full storage-mode support matrix (ExpertsNbit vs Experts4bit, compatibility, known limitations, the headline-number reading, reproduction + validation grids).
...
- **`OFFLOAD_EXPERTS=1`** keeps the frozen experts in pinned CPU RAM (set `OFFLOAD_PIN=0` to skip
  ...
  *what fits*, not how fast. Offloading changes tensor location, not math — unit-test-verified,
  ...
  [`docs/METHODOLOGY.md`](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/METHODOLOGY.md) §11.
...
[`docs/OFFLOAD-TRANSFER-NOTES.md`](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/OFFLOAD-TRANSFER-NOTES.md).
...
- **DeepSeek-V4 (Flash / Pro)** — per-expert **MXFP4** experts with a clamped-SwiGLU
  ...
  See [docs/DEEPSEEK-V4.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/DEEPSEEK-V4.md).
...
kill-switches, and the shape-dependence analysis: **[docs/INFERENCE.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/INFERENCE.md)**.
...
Moved to **[docs/SERVING.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/SERVING.md)** — The FastAPI serving shim + Docker deployment (endpoints, env knobs, the localhost-by-default / E4B_TOKEN posture).

## Benchmarks

Moved to **[docs/BENCHMARKS.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/BENCHMARKS.md)** — The benchmark scripts and how to run them (memory wall, tokens-per-joule, the upstream matmul_4bit comparison).

## Docs

| | |
|---|---|
| [CHOOSING.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/CHOOSING.md) | which mode to use, and why — the long form of the table above |
| [METHODOLOGY.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/METHODOLOGY.md) | hosts, protocols, every measurement's provenance |
| [STORAGE-MODES.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/STORAGE-MODES.md) | the nf4/fp4/int8/fp8/bf16/fp16 support matrix |
| [RESIDENCY-ENGINES.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/RESIDENCY-ENGINES.md) | the four engines, hot-set selection, host-regime laws |
| [INFERENCE.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/INFERENCE.md) | decode fast-paths, grids, shape-dependence |
| [DEEPSEEK-V4.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/DEEPSEEK-V4.md) | V4's storage split, epilogue, arena bake, key mapping |
| [SERVING.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/SERVING.md) | the FastAPI shim and Docker deployment |
| [BENCHMARKS.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/BENCHMARKS.md) | the benchmark scripts and how to run them |
| [BITSANDBYTES.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/BITSANDBYTES.md) | relationship to bitsandbytes, prior art |
- **[docs/ARCHITECTURE_SUPPORT.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/ARCHITECTURE_SUPPORT.md)** — which architectures load, run and capture, with the evidence bundle and the fixture-vs-real-checkpoint caveat.
...
Moved to **[docs/BITSANDBYTES.md](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/docs/BITSANDBYTES.md)** — How ExpertsNbit/Experts4bit relate to bitsandbytes #1965, the vendored-copy shim, and the prior-art credits.

## Provenance & audits

Every measured number above traces to a committed script/test and a named host, with receipts
under [`bench/`](https://github.com/pjordanandrsn/experts4bit-qlora/tree/v0.22.0/bench) and
[`docs/`](https://github.com/pjordanandrsn/experts4bit-qlora/tree/v0.22.0/docs) — cited inline at
each claim. **Scope note (2026-07-28):** [`PROVENANCE.md`](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/PROVENANCE.md)
...
[`.ots-history/`](https://github.com/pjordanandrsn/experts4bit-qlora/tree/v0.22.0/.ots-history/). Falsification work lives under [`audits/`](https://github.com/pjordanandrsn/experts4bit-qlora/tree/v0.22.0/audits/) — most
recently the audit of unsloth-zoo's MoE-4bit fix that produced unsloth-zoo#849/#850
([`audits/unsloth-zoo-4032/REPORT.md`](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/audits/unsloth-zoo-4032/REPORT.md)).

## License

MIT (see [LICENSE](https://github.com/pjordanandrsn/experts4bit-qlora/blob/v0.22.0/LICENSE)). `experts4bit_qlora/_vendor/experts.py` is vendored from
