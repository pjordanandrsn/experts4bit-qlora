...
| Gemma-4-26B-A4B-it | **HARNESS_ERROR** (receipt status `void_attn4`) — both arms died on the harness's projection-count check (`tp2_arm.py` asserts `n_attn4 == 4 · n_layers` on what the converter returns), `quantize_attention_projections_4bit converted 100 projections, expected 120` ([#412](https://github.com/pjordanandrsn/experts4bit-qlora/issues/412), fixed since by #435, and the arms have run since in P56 and tp4; not licensed: [#713](https://github.com/pjordanandrsn/experts4bit-qlora/issues/713)); the bf16-attention `fast_train` path stays as tp1 left it | OK · VALID — 60 expert stacks, the banner; 3.510 s/step at 20.824 GB (`…gemma4….arm.unsloth.ckpt_unsloth`) | not quoted (no e4b arm to pair) |
