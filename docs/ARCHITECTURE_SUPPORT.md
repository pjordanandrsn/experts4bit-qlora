...
| `ernie4_5_moe` | validated | **released checkpoint** — ERNIE-4.5-21B-A3B-PT, CPU, 2026-09-18 (`reference-ok`, Probe rows below); needed the checkpoint-key renaming fix and the MTP drop (#529; see notes) |
...
tensor by name. Re-run 2026-09-18 against the released ERNIE-4.5-21B-A3B-PT off the LAN (CPU, bf16, the 0.36.1
loader): the row below is `reference-ok` — all 9 shards integrity-clean, load 81.6 s, the 12 MTP
tensors skipped under the class declaration, 27 `moe_statics` keys renamed, 27 of 27 MoE layers
quantised (nf4), forward finite. The table row above is real-checkpoint evidence from that date;
CUDA-graph capture is untested for the family (CPU probe).
...
| Gemma-4-26B-A4B-it | **HARNESS_ERROR** (receipt status `void_attn4`) — both arms died on the harness's projection-count check (`tp2_arm.py` asserts `n_attn4 == 4 · n_layers` on what the converter returns), `quantize_attention_projections_4bit converted 100 projections, expected 120` ([#412](https://github.com/pjordanandrsn/experts4bit-qlora/issues/412), fixed since by #435; not re-run: [#703](https://github.com/pjordanandrsn/experts4bit-qlora/issues/703)); the bf16-attention `fast_train` path stays as tp1 left it | OK · VALID — 60 expert stacks, the banner; 3.510 s/step at 20.824 GB (`…gemma4….arm.unsloth.ckpt_unsloth`) | not quoted (no e4b arm to pair) |
...
`olmoe`, `qwen3_moe`, `mixtral`; not supported on `gemma4_text` pending a re-run (#703; the count-check failure, #412, is fixed by #435); refused on `gpt_oss` (bias-carrying
...
|---|---|---|---|---|---|---|---|---|
| `bailing_hybrid` | `Ling-3.0-tiny` | — | — | cpu | refused | — | — | **refused** |
| `deepseek_v2` | `hmellor/tiny-random-DeepseekV2ForCausalLM` | toy | convention | cpu | ok | 3q/0u | ok | **toy-ok** |
| `deepseek_v3` | `Moonlight-16B-A3B-Instruct` | — | convention | cpu | checkpoint-broken | — | — | **checkpoint-broken** |
| `deepseek_v4` | `DeepSeek-V4-Flash` | — | — | cpu | error | — | — | **host-limited** |
| `ernie4_5_moe` | `baidu/ERNIE-4.5-21B-A3B-PT` | reference | convention | cpu | ok | 27q/0u | ok | **reference-ok** |
...
| `granitemoehybrid` | `ibm-granite/granite-4.0-h-tiny` | reference | SUPPORTED_ARCHITECTURES | cpu | ok | 40q/0u | ok | **reference-ok** |
| `granitemoeshared` | `ibm-research/moe-7b-1b-active-shared-experts` | reference | SUPPORTED_ARCHITECTURES | cpu | ok | 40q/0u | ok | **reference-ok** |
| `jamba` | `ai21labs/Jamba-tiny-dev` | toy | SUPPORTED_ARCHITECTURES | cpu | ok | 8q/0u | ok | **toy-ok** |
| `lfm2_moe` | `LiquidAI/LFM2-8B-A1B` | reference | SUPPORTED_ARCHITECTURES | cpu | ok | 22q/0u | ok | **reference-ok** |
...
| `granitemoehybrid` | **reference-ok** | `ibm-granite/granite-4.0-h-tiny` |
| `granitemoeshared` | **reference-ok** | `ibm-research/moe-7b-1b-active-shared-experts` |
| `kimi_k3` | **none** | — |
| `deepseek_v4` | **host-limited** | `DeepSeek-V4-Flash` |
| `nemotron_h` | **reference-ok** | `nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-BF16` |
| `jamba` | **toy-ok** | `ai21labs/Jamba-tiny-dev` |
| `lfm2_moe` | **reference-ok** | `LiquidAI/LFM2-8B-A1B` |
...
- `bailing_hybrid` — refused on `Ling-3.0-tiny`
- `deepseek_v2` — toy-ok on `hmellor/tiny-random-DeepseekV2ForCausalLM`
- `deepseek_v3` — checkpoint-broken on `Moonlight-16B-A3B-Instruct`
