...
- **Granite-3.1-3B-A800M** — `r12epi`, NF4 experts + round-1/2 folds (rotary-only fold) + router epilogue (licensed:
  ...
  NF4 4.40 ms = 227.3)** and **×1.160 at B=16 (1836.8; NF4 1583.2) vs e4b's own NF4 control on the same box** — `e4b.serve.census.bo7.granite.b1.5090.2026-09-05` / `e4b.serve.census.bo7.granite.b16.5090.2026-09-05`.
  **No field comparator measured.** Beside, with their boxes: bo3 259.1 / 1689.6, bo5 294.1 / 1736.1 (both EPYC 9755 hosts).
- **OLMoE-1B-7B** — the position is **NF4** (×1.000 vs e4b's own NF4 control: 3.54 ms = 282.5 tok/s B=1; 1347.5 B=16), because nothing above
  ...
  **No field comparator measured.**
- **gpt-oss-20b** — the quoted best is the lane's own reference arm, `nf4_r12` (NF4 experts + exact folds; no raw-text
  instrument): ×1.000 vs e4b's own NF4 control, **6.92 ms = 144.5 tok/s B=1, 761.6 B=16** — `e4b.serve.census.bo7.gptoss.b1.5090.2026-09-05` / `e4b.serve.census.bo7.gptoss.b16.5090.2026-09-05`.
  **No field comparator measured.**
- **Qwen3-30B-A3B** — the licensed configuration, the streamed **64k** calibrated pack artifact (11512 gptq / 776 rtn) + calibrated int4 attention +
...
- **Gemma-4-26B-A4B** — no K8 instrument exists for this family, so no arm carries a K8 licence; the register's position
  ...
  103.6 tok/s; NF4 12.36 ms = 80.9)** and **×1.106 at B=16 (675.8; NF4 611.2) vs e4b's own NF4 control** — `e4b.serve.census.bo7.gemma4.b1.5090.2026-09-05` /
  `e4b.serve.census.bo7.gemma4.b16.5090.2026-09-05`. **No field comparator measured.** The register's quoted best `int4_r1epi` (bo3's `stack`) reads ×1.705 / ×1.697 (137.9 / 1037.4) measured with no
...
- **Mixtral-8x7B-Instruct** — the position is **NF4** (×1.000 vs e4b's own NF4 control: 19.87 ms = 50.3 tok/s B=1; 191.4 B=16;
  `e4b.serve.census.bo7.mixtral.b1.5090.2026-09-05` / `e4b.serve.census.bo7.mixtral.b16.5090.2026-09-05`; **no field comparator measured**): `lic` and `all` FAIL as registered on bo5, the calibrated stack FAILS on
...
Ratios on this box, every arm vs e4b's own NF4 control (B=1 / B=16); labels are the register's — no absolute is compared
across families:
