...
- **Attention q/k/v/o by STRUCTURE, not family name.** `add_attention_lora` and `quantize_attention_projections_4bit` share `detect_attention_projections`: `q_proj`/`k_proj`/`o_proj` must be supported linears; `v_proj` may be a supported linear or absent/`None` (Gemma-4 `attention_k_eq_v` layers). The expected count is `len(candidates)`, never `4 * n_layers`. A layer lacking `k_proj` is refused; a bias-carrying projection is refused (gpt-oss). The Gemma-4 attention-4-bit *configuration* stays not supported pending a measured lane ([#703](https://github.com/pjordanandrsn/experts4bit-qlora/issues/703)). This rule fixes the count check that killed tp2/P40's arms ([#412](https://github.com/pjordanandrsn/experts4bit-qlora/issues/412)), but it does not move that cell.
