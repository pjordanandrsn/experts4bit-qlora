...
- Families, per path and from evidence — the capability's `model_families` (`olmoe`, `qwen3_moe`, `gemma4_text`) is exactly the families whose `fast_train` cell below is `supported`:
