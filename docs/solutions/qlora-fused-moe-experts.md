...
### Head-to-head per family (lane tp2, 2026-09-06)

Claim family `e4b.train.h2h.unsloth.<family>.5090.2026-09-06` (measured, receipts in [`../../bench/h2h-20260906/tp2/`](../../bench/h2h-20260906/tp2/README.md); lane tp2, pre-registration `P40-PREREG.md` verbatim in the bundle): the same head-to-head as p38, on **all six families**, with the **shipped** cut — experts4bit-qlora 0.35.1 + grouped-nf4-gemm 0.30.2 from PyPI — against Unsloth 2026.9.2 + unsloth_zoo 2026.9.1, one rented RTX 5090 (Ryzen 7 5700X3D host, Vast 50005568), P38's fixture exactly, N = 60, every attempt a row. It quotes a speed position on two families and states why on the other four.

| family | position (Unsloth/e4b s/step) | quality at N=60 | what the row is |
|---|---|---|---|
| Qwen3-30B-A3B | **1.457** (5.986 vs 4.108 s — e4b faster per step) | COMPARABLE, Δ +0.0152 | `e4b.train.h2h.unsloth.qwen3.5090.2026-09-06`, `…quality-n60`; +3.1% against p38's 1.413 on another box, inside the pre-registered ±10% |
| Mixtral-8x7B-Instruct | **0.361** (0.858 vs 2.377 s — Unsloth faster per step) | COMPARABLE, Δ −0.0087 | `e4b.train.h2h.unsloth.mixtral.5090.2026-09-06` — read with `…footprint`: e4b under expert offload peaks at 3.223 GB against Unsloth resident at 29.163 GB (×9.05 less VRAM), and e4b is lower in energy (298.3 vs 350.2 J/step) |
| Granite-3.1-3B-A800M | not quoted | — | `…granite….coverage`: Unsloth trained the attention only (2,621,440 trainable against e4b's 49,807,360, no expert LoRA) so its arm is **VOID** and the arms are not the same problem |
| OLMoE-1B-7B-Instruct | not quoted | — | `…olmoe….coverage`: the Unsloth process exited rc=1 without a receipt (**HARNESS_ERROR**) while both e4b arms trained |
| gpt-oss-20b | not quoted | — | both sides **REFUSED**: e4b's expert path has nothing to patch and its attention-4-bit path refuses 96 of 96 biased projections; Unsloth raised on the checkpoint's automatic weight conversion at load |
| Gemma-4-26B-A4B-it | not quoted | — | e4b's attention-4-bit arms are **VOID_ATTN4** (`converted 100 projections, expected 120`, [#412](https://github.com/pjordanandrsn/experts4bit-qlora/issues/412)); the comparator's arm is VALID at 3.510 s/step, recorded as its observation |

**Mixtral is read footprint first.** Unsloth has no expert-offload mode, so the pair is e4b offloaded against Unsloth resident: a ×9.05 smaller peak at comparable held-out loss, bought with per-step time. That is the trade the registered design makes — the number is never quoted as a kernel ratio at equal residency, because no such arm was measured here — and P40's prediction that the resident arm would OOM on a 32 GB card is falsified. **The attention-4-bit configuration of the training paths gets its first per-family receipts here** (`reference_attn4` / `fused_attn4`): OK · VALID with the fused-vs-reference pair PASSING on Granite (×5.59), OLMoE (×3.73), Qwen3 (×2.69) and Mixtral (×1.23) — `…e4b-internal-parity`, informational, tp1 owns the licence — refused on gpt-oss, and **not supported on `gemma4_text`** pending #412, where the path without attention 4-bit stands as tp1 left it. Nothing in the lane licenses anything, and one box with one attempt per cell is the whole scope.
...
- "does e4b or Unsloth train a 4-bit Mixtral / Granite / OLMoE MoE on one GPU" / "per-family QLoRA comparison against Unsloth" — the tp2 table above, six families on one box.
...
- **Per family, the comparison is two positions and four stated reasons** (lane tp2, above). It does not say e4b is faster per step everywhere: on Mixtral, under the offload that makes the family fit at all, Unsloth resident is faster per step (`e4b.train.h2h.unsloth.mixtral.5090.2026-09-06`, ratio 0.361) and the e4b row's win is footprint and energy (`…footprint`). Granite, OLMoE, gpt-oss and Gemma-4 carry no ratio, each for a recorded reason, and none of those reasons is an "unsupported" flag against the comparator. Nothing in the lane is a convergence claim, a cross-family ratio or a cross-box ratio — its two lanes' Qwen3 ratios agree (1.457 against 1.413) while their absolutes do not travel.
- **Attention 4-bit is a separate cell per family, and it is not supported on Gemma-4.** `TRAIN_ATTN_4BIT` (`quantize_attention_projections_4bit`) has receipts on `granitemoe`, `olmoe`, `qwen3_moe` and `mixtral` (tp2's `reference_attn4` / `fused_attn4` arms); on `gemma4_text` it converted 100 of the expected 120 projections and both arms were written VOID_ATTN4 ([#412](https://github.com/pjordanandrsn/experts4bit-qlora/issues/412)), so the configuration is refused there until that is fixed while the same paths *without* attention 4-bit stand as tp1 measured them; on `gpt_oss` it refuses because every attention projection carries a bias.
...
- `e4b.train.h2h.unsloth.qwen3.5090.2026-09-06` and `e4b.train.h2h.unsloth.mixtral.5090.2026-09-06` — the same head-to-head per family on the shipped cut (lane tp2): the two positions, each with its `…quality-n60`; `e4b.train.h2h.unsloth.mixtral.5090.2026-09-06.footprint` — the offloaded peak against Unsloth resident on one 32 GB card; `e4b.train.h2h.unsloth.granite.5090.2026-09-06.coverage` and `e4b.train.h2h.unsloth.olmoe.5090.2026-09-06.coverage` — the two families where the comparator could not train the experts, recorded as results; `e4b.train.h2h.unsloth.<family>.5090.2026-09-06.e4b-internal-parity` — the fused-vs-reference PASS with attention in 4-bit on Granite, OLMoE, Qwen3 and Mixtral (informational); one row per attempt under `e4b.train.h2h.unsloth.<family>.5090.2026-09-06.arm.*`, refusals and VOID_ATTN4 rows included.
