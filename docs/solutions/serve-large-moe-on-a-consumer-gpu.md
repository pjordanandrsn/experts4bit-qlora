...
- Not a vLLM replacement: on the same box with identical prompts vLLM is ahead (claim `e4b.serve.h2h.vllm.same-box`, measured-private — the e4b arm of that comparison is the 2026-09-03 round-to-nearest int4 class, not the licensed stack, and the vLLM version is not recorded in its receipt; the comparison against the licensed stack is lane P37, registered when its receipt lands).
