...
- **Gemma-4-26B-A4B** (~13 GB experts): likewise ≈ *13 GB / n_layers* resident, well under the card — but a **pure projection**: the loader does not yet support Gemma-4 (experts + router are laid out differently).

This is the same extrapolation move §9c makes (op-level → full model), and it is an **estimate** until run. The loader now supports **Qwen3-MoE** (`SUPPORTED_MODEL_TYPES`), so **Qwen3-30B-A3B** is loadable today and offload makes it fit 12 GB directly — a *measured* number just needs the run; Gemma-4 stays a projection until its loader lands.

**Verdict on offload:** a capacity feature — it decides *what fits*, at a PCIe throughput cost (the honest framing this repo already applies to 4-bit itself). Correctness (offload = location, not math, *through the gradient-checkpoint recompute*) is **proven by the unit tests** (8/8 on an RTX A2000); the OLMoE peak-GPU-drop / s-step-cost **A/B is specified in §11a but not yet run** — `bench/run-offload-ab.sh` on the 12 GB card fills the table; the 26–35B benefit is a **labelled projection** (one-layer residency ≪ 12 GB) — now directly checkable on **Qwen3-30B-A3B**, which the loader supports (Gemma-4 still awaits its own loader).
