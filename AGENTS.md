...
- **GPU rental experiments follow the compute governance policy**:
  approval thresholds (self-approval ≤ $2; one-of CTO/CSO ≤ $20;
  two-of CEO/CTO/CSO ≤ $50; Jordan above), per-role daily ceilings,
  a global daily budget, teardown proof required, immutable receipts
  at `bench/runs/<date>/<run-id>/receipt.json` validated against
  [`docs/run-receipt-schema.json`](docs/run-receipt-schema.json).
  The policy is [`docs/COMPUTE-GOVERNANCE.md`](docs/COMPUTE-GOVERNANCE.md);
  `scripts/check_run_ledger.py` enforces it in CI. Rent through
  `python -m experts4bit_qlora.tools.rent` (or `scripts/rent_run.py`): it
  checks the ledger and approvals before create, arms a controller-side
  teardown guard, and writes the receipt.
...
- The taking agent acknowledges on the issue, works on a branch, opens a pull
  ...
  same diff. Nobody merges their own PR. The gate holder reads the PR's base branch before merging and confirms the squash commit is on `main` afterwards; a stacked PR is retargeted with `gh pr edit --base main` (or its base branch deleted) before merge. Receipts
...
- A task pull request never moves a gate, a threshold, the `[fast]` floor or a
  registered claim; if the task needs one, the agent stops and says so on the
  issue.
- Sections 2–9 bind every agent equally: structure not names, counts or
  raise, measured means a receipt in the repository, `llms-full.txt`
