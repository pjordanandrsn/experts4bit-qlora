#!/bin/bash
# bench/p70/p70_run.sh -- lane P70, BOX side (bench/p70/P70-PREREG.md; e4b#726). Derived from bench/p64/p64_run.sh by
# named substitutions only: the preflight, install, tripwire, scorer self-test, K0, pack build and fingerprint check are
# P64's; the scorer is kl_router.py (P64's kl_a16.py imported unchanged, its pass table / toggle / counts replaced), the
# passes are P70's, the NF4 anchor is dropped (P70 reads one stack against itself), and the reducer is p70_reduce.py.
# Started detached by p70_drive.sh with the run's nonce; P70_RUN_NONCE first, then P70_EXIT_CODE.<nonce> + TP_DONE.<nonce>
# on every exit and P70_SUCCESS.<nonce> only when the registered read can be made.
#
# Knobs (every one recorded in summary.txt; any value off its registered default marks the run a REHEARSAL, NOT a
# reading): P70_MODEL P70_REVISION P70_LICENSED_FP P70_GPU_CLASS P70_MIN_DISK_GB P70_CALIB_NSEQ P70_HESSIAN_BUDGET_GB
# P70_BUILD_PPL_STEPS P70_BUILD_VIA P70_ROWS P70_SKIP_INSTALL P70_REHEARSAL.
# P70_PROVE=1 is the PROVING RUN (P70-PREREG "Box and cost"): refusals, the install at the pins, the tripwire, the
# scorer's self-test, K0, the cast on this card's real router kernel and an egress probe -- no model, no pack; exit 0.
set -uo pipefail
W=/root/p70; mkdir -p $W/logs; cd $W || exit 78
say(){ echo "[$(date -u +%FT%TZ)] p70: $*"; }
NONCE=${P70_RUN_NONCE:?}; printf '%s\n' "$NONCE" > $W/P70_RUN_NONCE.tmp && mv $W/P70_RUN_NONCE.tmp $W/P70_RUN_NONCE
finish(){ local rc=$1; printf '%s\n' "$rc" > P70_EXIT_CODE.$NONCE; [ "$rc" = 0 ] && : > P70_SUCCESS.$NONCE; say "TP_DONE rc=$rc"; : > TP_DONE.$NONCE; exit "$rc"; }
trap 'finish 130' INT TERM
for v in P70_RUN_ID P70_DEADLINE_EPOCH P70_INSTANCE_ID E4B_SHA GNF4_SHA; do [ -n "${!v:-}" ] || { say "refusing: $v unset"; finish 78; }; done
...
# ---- registered defaults (P70-PREREG "Box and cost"); a rehearsal overrides them and says so
...
MID=${P70_MODEL:-$D_MODEL}; REV=${P70_REVISION:-$D_REV}; LIC_FP=${P70_LICENSED_FP:-$D_FP}
GPU_CLASS=${P70_GPU_CLASS:-5090}; MIN_DISK_GB=${P70_MIN_DISK_GB:-200}; NSEQ=${P70_CALIB_NSEQ:-128}
HBUDGET=${P70_HESSIAN_BUDGET_GB:-24}; BUILD_STEPS=${P70_BUILD_PPL_STEPS:-2048}; BUILD_VIA=${P70_BUILD_VIA:-step_decomp}
ROWS=${P70_ROWS:-16}; SKIP_INSTALL=${P70_SKIP_INSTALL:-0}; REHEARSAL=${P70_REHEARSAL:-0}
...
unset E4B_INT4_DECODE_A16 E4B_ROUTER_EPI_CAST    # the served stack is built with the shipped defaults; kl_router.py flips the cast per pass
: > summary.txt; echo "$P70_INSTANCE_ID" > INSTANCE_ID
...
for f in kl_router.py p70_reduce.py kl_a16.py p64_reduce.py kl_b16.py p59_reduce.py serve_stack.py kl_fidelity.py step_decomp.py k8_bake.py calib.json hook/usercustomize.py staged.sha256; do
  [ -s $W/$f ] || { say "STAGE MISSING: $f"; finish 9; }; done
(cd $W && sha256sum -c staged.sha256 >/dev/null) || { say "STAGED FILES DIFFER FROM bench/p70/staged.sha256"; finish 9; }
...
can_run(){ local need=$1 now; now=$(date +%s); [ $((now + need + 600)) -le "$P70_DEADLINE_EPOCH" ] || { say "STOP: $2 needs ${need}s, only $((P70_DEADLINE_EPOCH - now))s left -- skipped (host-limited)"; echo "SKIPPED $2 host-limited deadline" >> summary.txt; return 1; }; }
arm_alarm(){ local left=$(( P70_DEADLINE_EPOCH - $(date +%s) - 600 )); [ "$left" -lt 600 ] && left=600; echo "$left"; }
...
if [ "$SKIP_INSTALL" = 0 ]; then
  ...
  say "install SKIPPED (P70_SKIP_INSTALL=1): the importable packages are whatever the environment provides -- recorded below"
fi
...
assert hasattr(hr, "DECODE_A16") and hasattr(hr, "_decode_a16_default"), "e4b cut lacks E4B_INT4_DECODE_A16 (P64), which P70's scorer holds off"
assert hr.DECODE_A16[0] is False, "DECODE_A16 is on at import: the served stack must be built with the shipped default"
from experts4bit_qlora.engines import router_epilogue as rep_
assert hasattr(rep_, "CAST_WEIGHTS") and hasattr(rep_, "_cast_default"), "e4b cut lacks E4B_ROUTER_EPI_CAST (P70): the lever cannot run"
assert rep_.CAST_WEIGHTS[0] is False, "CAST_WEIGHTS is on at import: the served stack must be built with the shipped default"
...
open("/root/p70/versions.txt", "w").write(f"e4b {e.__version__} @{os.environ['E4B_SHA']} (from {os.path.dirname(e.__file__)})\ngnf4 {gnf4} @{os.environ['GNF4_SHA']} (int4_b32 from {int4_b32.__file__})\n"
...
perl -e 'alarm 600; exec @ARGV' python $W/kl_router.py --self-test > logs/selftest.log 2>&1 || { tail -5 logs/selftest.log; say "SCORER SELF-TEST FAILED"; finish 21; }
...
if [ "${P70_PROVE:-0}" = 1 ]; then
  echo "PROVE -- the proving run: no model, no pack, no KL row" | tee -a summary.txt
  say "PROVE: the cast on this card's real router kernel (kl_router.py --prove-cast)"
  perl -e 'alarm 300; exec @ARGV' python -u $W/kl_router.py --prove-cast --out $W/prove_cast.json > logs/prove_cast.log 2>&1; prc=$?
  grep -a "PROVECAST" logs/prove_cast.log | cut -c1-600 | tee -a summary.txt
  [ "$prc" = 0 ] || { tail -5 logs/prove_cast.log; say "PROVE: the cast check FAILED (rc=$prc)"; finish 27; }
...
for src, label in (("wikitext", "wikitext-2-raw-v1 test via step_decomp._k8_window"), ("c4val1", "allenai/c4 en/c4-validation.00001-of-00008 (first 2000 docs) via step_decomp._k8_window")):
    ...
    json.dump(rec, open(f"/root/p70/prompts_{src}.json", "w"))
...
first_chunk_watchdog(){ local pid=$1 log=$2 budget=${P70_FIRST_CHUNK_S:-1500} t0; t0=$(date +%s)
...
{ echo "P70 build via=$BUILD_VIA at=$(date -u +%FT%TZ)"; } > logs/build.log
...
if can_run 1800 served; then
  say "served stack: every pass (kl_router.py; deadline-aware inside)"
  env $LOAD E4B_SERVE_ATTN_INT4_CALIB=1 E4B_SERVE_ATTN_INT4=0 E4B_CALIB_SOURCE=c4 E4B_FUSE_T1_GLUE=1 E4B_FUSE_T1_GLUE_R2=1 E4B_FUSE_ROUTER_EPI=1 \
    perl -e "alarm $(arm_alarm); exec @ARGV" python -u $W/kl_router.py --model "$MID" --arena "$QA" --calib $W/calib.json --prompts "$PROMPTS" \
      --rows $ROWS --prefix 384 --out $W/out --deadline-epoch $P70_DEADLINE_EPOCH --margin-s 900 > logs/run_served.log 2>&1 || note $?
...
say "reduce"; python $W/p70_reduce.py $W --md $W/RESULTS-p70-generated.md --json $W/p70_rep.json > logs/reduce.log 2>&1 || { tail -5 logs/reduce.log; note 43; }
grep -aE "^\*\*(VALIDITY|VERDICT)|^- \*\*" $W/RESULTS-p70-generated.md 2>/dev/null | cut -c1-240 | tee -a summary.txt
...
MISSING=""
for p in ep32 ep16 ep32_rep ep32_pc96; do for t in wikitext c4val1; do
...
python -c "import json,sys; r=json.load(open('$W/p70_rep.json')); v=r.get('validity',{}).get('status'); sys.exit(0 if v=='VALID' else 1)" 2>/dev/null || { echo "VALIDITY NOT MET (see RESULTS-p70-generated.md)" >> summary.txt; note 41; }
