#!/bin/bash
# bench/p70/p70_drive.sh -- lane P70, CONTROLLER side: the launcher's --command (bench/p70/P70-PREREG.md; e4b#726).
# Derived from bench/p64/p64_drive.sh by named substitutions: it also stages P64's kl_a16.py and p64_reduce.py,
# which P70's scorer and reducer import unchanged.
# Stages P70's scorer, reducer and runner (and P64's scorer and reducer, which they import) beside the pieces they reuse (P59's scorer and reducer, P44's serve_stack,
...
P64="$REPO/bench/p64"
STAGE="$HERE/p70_run.sh $HERE/kl_router.py $HERE/p70_reduce.py $P64/kl_a16.py $P64/p64_reduce.py $P59/kl_b16.py $P59/p59_reduce.py $P44/serve_stack.py $BENCH/kl_fidelity.py $P39/step_decomp.py $P39/k8_bake.py $P39/calib.json $HERE/staged.sha256"
...
# (the same case as tests/test_p70_staged_pin.py, which runs this check in CI where it costs nothing).
sha_of(){ (sha256sum "$1" 2>/dev/null || shasum -a 256 "$1") | cut -d" " -f1; }
while read -r want name; do
  case "$want" in \#*|"") continue;; esac
  case "$name" in
    p70_run.sh|kl_router.py|p70_reduce.py) src="$HERE/$name";;
    kl_a16.py|p64_reduce.py) src="$P64/$name";;
...
GNF4_SHA=${GNF4_SHA:?set GNF4_SHA to the registered grouped-nf4-gemm cut (P70-PREREG: 5ca1897585f9f456f99ea504b2a1be0ea91db496, v0.33.0, the P64 cut)}
...
PASS="P70_RUN_ID=$RUN_ID P70_RUN_NONCE=$NONCE P70_DEADLINE_EPOCH=$DEADLINE P70_INSTANCE_ID=$E4B_RENT_INSTANCE_ID E4B_SHA=$E4B_SHA GNF4_SHA=$GNF4_SHA"
...
# marks it REHEARSAL. P70_PROVE=1 is the proving rental the pre-registration names (no model, no pack, no KL).
for v in P70_FIRST_CHUNK_S P70_PROVE; do [ -n "${!v:-}" ] && PASS="$PASS $v=$(printf %q "${!v}")"; done
if [ "${P70_DRIVE_DRYRUN:-0}" = "1" ]; then echo "DRYRUN stage -> root@$HOST:$W ; start: env $PASS bash p70_run.sh ; poll TP_DONE.$NONCE until $DEADLINE ; fetch -> $RUN_DIR/p70"; exit 0; fi
say "run $RUN_ID nonce=$NONCE -> $HOST:$PORT; e4b $E4B_SHA (from $REPO); gnf4 $GNF4_SHA; receipts -> $RUN_DIR/p70; deadline $DEADLINE"
...
$SSH "cd $W || exit 20; nohup env $PASS bash p70_run.sh > outer.log 2>&1 < /dev/null & child=\$!; end=\$((\$(date +%s)+30)); while [ \$(date +%s) -lt \$end ]; do [ \"\$(cat P70_RUN_NONCE 2>/dev/null)\" = '$NONCE' ] && { echo started:\$child; exit 0; }; kill -0 \$child 2>/dev/null || { wait \$child; echo child-exited-early:rc=\$? >&2; exit 125; }; sleep 1; done; echo nonce-handshake-timeout >&2; exit 124" || { say "start failed: child did not bind the nonce"; exit 21; }
...
STALL_S=${P70_STALL_S:-900}; P70_MIN_PROGRESS_MB=${P70_MIN_PROGRESS_MB:-16}
progress_verdict() {  # idle_s stall_s util dfk_now dfk_prev du_now du_prev -> "" | "fetching:<delta>" | "stall:<idle_s>"
  idle_s=$1; stall_s=$2; util=$3; dfk_now=$4; dfk_prev=$5; du_now=$6; du_prev=$7; min_mb=$P70_MIN_PROGRESS_MB
  consumed=0; grew=0
...
while :; do
  ...
  hb=$($SSH "echo \"\$(grep -v '^[[:space:]]*$' $W/summary.txt 2>/dev/null | tail -n 1 | cut -c1-160) | gpu \$(nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits 2>/dev/null | tr -d ' ') | du \$(du -sm $W 2>/dev/null | cut -f1)M | disk \$(df -h /root | tail -1 | awk '{print \$4}') | dfk \$(df -k /root | tail -1 | awk '{print \$4}') | live \$(pgrep -f 'bash p70_run.sh' | wc -l | tr -d ' ')\"" 2>/dev/null)
  ...
  if [ -n "$(lane_dead "$live" "$LAST_LIVE")" ]; then
    say "LANE DEAD: no 'bash p70_run.sh' on the box for two consecutive polls and no TP_DONE -- not waiting out the deadline"
    LANE_DEAD=1; break
  fi
  LAST_LIVE=$live
...
rm -rf "$RUN_DIR/p70" && mkdir -p "$RUN_DIR/p70" || { say "fetch failed: local dir"; exit 22; }
...
[ "$(cat "$RUN_DIR/p70/P70_RUN_NONCE" 2>/dev/null)" = "$NONCE" ] || { say "stale or foreign nonce in fetched artifacts"; exit 24; }
[ -f "$RUN_DIR/p70/TP_DONE.$NONCE" ] || {
  [ "${LANE_DEAD:-0}" = 1 ] && { say "lane DIED on the box: its process was gone with no P70_EXIT_CODE and no TP_DONE -- artifacts fetched, nothing measured"; exit 25; }
  say "lane did not finish (no TP_DONE for this run)"; exit 23; }
RC=$(cat "$RUN_DIR/p70/P70_EXIT_CODE.$NONCE" 2>/dev/null); case "$RC" in ''|*[!0-9]*) say "malformed exit code"; exit 24;; esac
[ -f "$RUN_DIR/p70/P70_SUCCESS.$NONCE" ] && [ "$RC" = 0 ] || { say "lane exit rc=$RC without success marker"; exit "$RC"; }
