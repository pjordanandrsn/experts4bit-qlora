#!/bin/bash
# bench/p68/p68_drive.sh -- lane P68, CONTROLLER side: the launcher's --command (bench/p68/P68-PREREG.md). Stages the
# probe and reducer, P63's probe, comparison module and fixture (reused unchanged), P64's committed wikitext rows (the
# size reading's text), P44's serve_stack.py and P39's k8_bake.py + calib.json, plus the HF token (never on a command
# line); starts p68_run.sh detached under a fresh nonce, polls TP_DONE.<nonce> with the
...
set -uo pipefail
say(){ echo "[$(date -u +%FT%TZ)] [p68_drive] $*"; }
...
P39="$REPO/bench/p39"; P44="$REPO/bench/p44"; P63="$REPO/bench/p63"; P64R="$REPO/bench/p64/receipts"
STAGE="$HERE/p68_run.sh $HERE/p68_probe.py $HERE/p68_reduce.py $P63/p63_probe.py $P63/p63_compare.py $P63/fixture.txt $P64R/prompts_wikitext.json $P44/serve_stack.py $P39/k8_bake.py $P39/calib.json $HERE/staged.sha256"
...
while read -r want name; do
  ...
  case "$name" in p68_run.sh|p68_probe.py|p68_reduce.py) src="$HERE/$name";; p63_probe.py|p63_compare.py|fixture.txt) src="$P63/$name";; prompts_wikitext.json) src="$P64R/$name";; serve_stack.py) src="$P44/$name";; *) src="$P39/$name";; esac
  got=$(sha_of "$src")
...
[ -n "${GNF4_SHA:-}" ] || { say "refusing: GNF4_SHA is not set -- pass the launch manifest's grouped-nf4-gemm cut"; exit 78; }
...
PASS="P68_RUN_ID=$RUN_ID P68_RUN_NONCE=$NONCE P68_DEADLINE_EPOCH=$DEADLINE P68_INSTANCE_ID=$E4B_RENT_INSTANCE_ID E4B_SHA=$E4B_SHA GNF4_SHA=$GNF4_SHA"
if [ "${P68_DRIVE_DRYRUN:-0}" = "1" ]; then echo "DRYRUN stage -> root@$HOST:$W ; start: env $PASS bash p68_run.sh ; poll TP_DONE.$NONCE until $DEADLINE ; fetch -> $RUN_DIR/p68"; exit 0; fi
say "run $RUN_ID nonce=$NONCE -> $HOST:$PORT; e4b $E4B_SHA (from $REPO); gnf4 $GNF4_SHA; receipts -> $RUN_DIR/p68; deadline $DEADLINE"
...
$SSH "cd $W || exit 20; nohup env $PASS bash p68_run.sh > outer.log 2>&1 < /dev/null & child=\$!; end=\$((\$(date +%s)+30)); while [ \$(date +%s) -lt \$end ]; do [ \"\$(cat P68_RUN_NONCE 2>/dev/null)\" = '$NONCE' ] && { echo started:\$child; exit 0; }; kill -0 \$child 2>/dev/null || { wait \$child; echo child-exited-early:rc=\$? >&2; exit 125; }; sleep 1; done; echo nonce-handshake-timeout >&2; exit 124" || { say "start failed: child did not bind the nonce"; exit 21; }
...
STALL_S=${P68_STALL_S:-900}; P68_MIN_PROGRESS_MB=${P68_MIN_PROGRESS_MB:-16}
progress_verdict() {  # idle_s stall_s util dfk_now dfk_prev du_now du_prev -> "" | "fetching:<delta>" | "stall:<idle_s>"
  idle_s=$1; stall_s=$2; util=$3; dfk_now=$4; dfk_prev=$5; du_now=$6; du_prev=$7; min_mb=$P68_MIN_PROGRESS_MB
  consumed=0; grew=0
...
while :; do
  ...
  hb=$($SSH "echo \"\$(grep -v '^[[:space:]]*$' $W/summary.txt 2>/dev/null | tail -n 1 | cut -c1-160) | gpu \$(nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits 2>/dev/null | tr -d ' ') | du \$(du -sm $W 2>/dev/null | cut -f1)M | disk \$(df -h /root | tail -1 | awk '{print \$4}') | dfk \$(df -k /root | tail -1 | awk '{print \$4}') | live \$(pgrep -f 'bash p68_run.sh' | wc -l | tr -d ' ')\"" 2>/dev/null)
  ...
  if [ -n "$(lane_dead "$live" "$LAST_LIVE")" ]; then
    say "LANE DEAD: no 'bash p68_run.sh' on the box for two consecutive polls and no TP_DONE -- not waiting out the deadline"
    LANE_DEAD=1; break
  fi
  LAST_LIVE=$live
...
rm -rf "$RUN_DIR/p68" && mkdir -p "$RUN_DIR/p68" || { say "fetch failed: local dir"; exit 22; }
...
rsync -az -e "ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new -p $PORT" --include 'work/' --include 'work/bake.json' --exclude 'work/*' --exclude 'src' --exclude 'venv*' --exclude '.cache' "root@$HOST:$W/" "$RUN_DIR/p68/" || { say "fetch failed: rsync"; exit 22; }
say "fetched $(ls "$RUN_DIR/p68" | wc -l | tr -d ' ') entries"
[ "$(cat "$RUN_DIR/p68/P68_RUN_NONCE" 2>/dev/null)" = "$NONCE" ] || { say "stale or foreign nonce in fetched artifacts"; exit 24; }
[ -f "$RUN_DIR/p68/TP_DONE.$NONCE" ] || {
  [ "${LANE_DEAD:-0}" = 1 ] && { say "lane DIED on the box: its process was gone with no P68_EXIT_CODE and no TP_DONE -- artifacts fetched, nothing measured"; exit 25; }
  say "lane did not finish (no TP_DONE for this run)"; exit 23; }
RC=$(cat "$RUN_DIR/p68/P68_EXIT_CODE.$NONCE" 2>/dev/null); case "$RC" in ''|*[!0-9]*) say "malformed exit code"; exit 24;; esac
[ -f "$RUN_DIR/p68/P68_SUCCESS.$NONCE" ] && [ "$RC" = 0 ] || { say "lane exit rc=$RC without success marker"; exit "$RC"; }
