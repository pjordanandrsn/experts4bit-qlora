#!/bin/bash
# bench/p44/p44b_drive.sh -- lane P44-b, CONTROLLER side: the launcher's --command. Stages the KL instrument
# (bench/kl_fidelity.py, kl_paths.py, kl_prompts.py), P39's k8_bake.py + calib.json, P42's hook, and this lane's
# serve_stack.py / kl_serve.py / p44b_run.sh; starts the lane detached under a fresh nonce; polls TP_DONE.<nonce>; fetches receipts. Nothing here
# creates, destroys or approves compute. Pattern: bench/p42/p42_drive.sh.
set -uo pipefail
say(){ echo "[$(date -u +%FT%TZ)] [p44b_drive] $*"; }
...
KL="$REPO/bench"
STAGE="$HERE/p44b_run.sh $HERE/serve_stack.py $HERE/kl_serve.py $KL/kl_fidelity.py $KL/kl_paths.py $KL/kl_prompts.py $P39/k8_bake.py $P39/calib.json $HERE/staged-b.sha256"
...
while read -r want name; do
  ...
  case "$name" in hook/*) src="$REPO/bench/p42/$name";; p44b_run.sh|serve_stack.py|kl_serve.py) src="$HERE/$name";; kl_*.py) src="$KL/$name";; *) src="$P39/$name";; esac
  got=$(sha_of "$src"); [ "$got" = "$want" ] || { say "refusing: $src is $got, staged-b.sha256 says $want"; exit 78; }
done < "$HERE/staged-b.sha256"
if [ -z "${E4B_SHA:-}" ]; then
...
$SSH "cd $W || exit 20; nohup env $PASS bash p44b_run.sh > outer.log 2>&1 < /dev/null & child=\$!; end=\$((\$(date +%s)+30)); while [ \$(date +%s) -lt \$end ]; do [ \"\$(cat P44B_RUN_NONCE 2>/dev/null)\" = '$NONCE' ] && { echo started:\$child; exit 0; }; sleep 1; done; echo 'no nonce handshake in 30 s'; exit 21" || { say "lane did not start"; exit 21; }
...
rm -rf "$RUN_DIR/p44b" && mkdir -p "$RUN_DIR/p44b" || { say "fetch failed: local dir"; exit 22; }
rsync -az -e "ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new -p $PORT" --exclude 'work_*' --exclude 'refcache_*' --exclude '.cache' --exclude '__pycache__' "root@$HOST:$W/" "$RUN_DIR/p44b/" || { say "fetch failed: rsync"; exit 22; }
say "fetched $(ls "$RUN_DIR/p44b" | wc -l | tr -d ' ') entries"
[ "$(cat "$RUN_DIR/p44b/P44B_RUN_NONCE" 2>/dev/null)" = "$NONCE" ] || { say "stale or foreign nonce in fetched artifacts"; exit 24; }
[ -f "$RUN_DIR/p44b/TP_DONE.$NONCE" ] || { say "lane did not finish (no TP_DONE for this run)"; exit 23; }
RC=$(cat "$RUN_DIR/p44b/P44B_EXIT_CODE.$NONCE" 2>/dev/null); case "$RC" in ''|*[!0-9]*) say "malformed exit code"; exit 24;; esac
[ -f "$RUN_DIR/p44b/P44B_SUCCESS.$NONCE" ] && [ "$RC" = 0 ] || { say "lane exit rc=$RC without success marker"; exit "$RC"; }
