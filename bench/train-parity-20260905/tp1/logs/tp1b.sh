...
# amendment 5 (15:50Z): the dataset + anchor variables the arm() helper needs (the lane's dataset block was not copied; run 1 died on `DATA: unbound variable`)
DATA=$W/data/ds_$DATASET.json
DATA_SHA=$(python -c "import json,sys; print(json.load(open('$W/ds_manifest.json'))['$DATASET']['sha256'])")
GOT_SHA=$(sha256sum $DATA | awk '{print $1}'); [ "$GOT_SHA" = "$DATA_SHA" ] || { echo "DATASET MISMATCH: $GOT_SHA != $DATA_SHA"; touch TP2_DONE; exit 13; }
export TP1_ANCHOR_JSON=$W/anchor.json TP1_BOX_CLASS="$(grep -E '^\s*class ' $W/logs/anchor_gate.log 2>/dev/null | awk '{print $2}')"
...
# amendment 4 (14:15Z): redo any family whose reference receipt is missing (a fetch that alarmed out), XET disabled, same fixture and alarms
if [ ! -s $W/gemma4_train_reference.json ]; then say "tp1b: Gemma-4 redo (no reference receipt)"; GM=google/gemma-4-26B-A4B-it
  fetch $GM gemma4 4800 && { arm gemma4 reference 3000 $GM 0; arm gemma4 fused 2400 $GM 0; arm gemma4 batched 3000 $GM 0; echo "GEMMA4 DONE"; }
  free_family gemma4 google--gemma-4-26B-A4B-it 2>/dev/null || true; fi
if [ ! -s $W/mixtral_train_reference.json ]; then say "tp1b: Mixtral redo (no reference receipt)"; MX=mistralai/Mixtral-8x7B-Instruct-v0.1
  fetch $MX mixtral 7200 && { arm mixtral reference 4200 $MX 1; arm mixtral fused 3600 $MX 1; arm mixtral batched 4200 $MX 1; echo "MIXTRAL DONE"; }
  free_family mixtral mistralai--Mixtral-8x7B-Instruct-v0.1 2>/dev/null || true; fi
