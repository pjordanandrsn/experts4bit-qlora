#!/bin/bash
# bench/k16/k16p5_run.sh -- lane K16 P5, BOX side (grouped-nf4-gemm kernel/PREREG-k16-smallm-int4-gemm.md, P5). Started
# detached by k16p5_drive.sh with the run's nonce; K16P5_RUN_NONCE first, then K16P5_EXIT_CODE.<nonce> + TP_DONE.<nonce> on
# every exit and K16P5_SUCCESS.<nonce> only on clean completion.
#
# P42's census runner (step_decomp.py --replay-profile-out, the timed B=16 window untouched by the census) with THREE
# arms on one RTX 5090: `nf4_b16` (control), `int4_b16` (RTN int4 experts + uncalibrated int4 attention -- P42's arm,
# whose rows > 1 take Int4Linear's cached-bf16 matmul), and `int4_b16_smallm` (the SAME bytes with
# E4B_ATTN_INT4_SMALLM=1: rows 2..16 take grouped-nf4-gemm's int4_smallm GEMM, no bf16 cache). P5 is read from the
# census: the attention-projection GEMM rows must fall by >= 0.4 ms/step between int4_b16 and int4_b16_smallm (the
# reduce script names the kernels); the timed step_ms_clean is recorded beside it. Nothing here changes a default.
set -uo pipefail
W=/root/k16p5; mkdir -p $W/logs; cd $W
say(){ echo "[$(date -u +%FT%TZ)] k16p5: $*"; }
NONCE=${K16P5_RUN_NONCE:?}; printf '%s\n' "$NONCE" > $W/K16P5_RUN_NONCE.tmp && mv $W/K16P5_RUN_NONCE.tmp $W/K16P5_RUN_NONCE
finish(){ local rc=$1; printf '%s\n' "$rc" > K16P5_EXIT_CODE.$NONCE; [ "$rc" = 0 ] && : > K16P5_SUCCESS.$NONCE; say "TP_DONE rc=$rc"; : > TP_DONE.$NONCE; exit "$rc"; }
trap 'finish 130' INT TERM
for v in K16P5_RUN_ID K16P5_DEADLINE_EPOCH K16P5_INSTANCE_ID E4B_SHA GNF4_SHA; do [ -n "${!v:-}" ] || { say "refusing: $v unset"; finish 78; }; done
...
: > summary.txt; echo "$K16P5_INSTANCE_ID" > INSTANCE_ID
# ---- staged pieces, byte-for-byte
for f in step_decomp.py k8_bake.py calib.json hook/usercustomize.py staged-p5.sha256; do [ -s $W/$f ] || { say "STAGE MISSING: $f"; finish 9; }; done
(cd $W && sha256sum -c staged-p5.sha256 >/dev/null) || { say "STAGED FILES DIFFER FROM bench/k16/staged-p5.sha256"; finish 9; }
# ---- deadline guard: never start an arm that cannot finish 10 min before the launcher tears the box down
can_run(){ local need=$1 now; now=$(date +%s); [ $((now + need + 600)) -le "$K16P5_DEADLINE_EPOCH" ] || { say "STOP-2: $2 needs ${need}s, only $((K16P5_DEADLINE_EPOCH - now))s left -- skipped (host-limited)"; echo "SKIPPED $2 host-limited deadline" >> summary.txt; return 1; }; }
...
arm_alarm(){ local left=$(( K16P5_DEADLINE_EPOCH - $(date +%s) - 600 )); [ "$left" -lt 1800 ] && left=1800; [ "$left" -gt 18000 ] && left=18000; echo "$left"; }
...
# calibrated arm is killed early if chunk 1 does not land within K16P5_FIRST_CHUNK_S (default 1500 s, ~3x
...
first_chunk_watchdog(){ local pid=$1 name=$2 budget=${K16P5_FIRST_CHUNK_S:-1500} t0; t0=$(date +%s)
...
hdr(){ { echo "K16P5 arm=$1 gnf4=${GNF4_LIVE:-?} sha=${GNF4_LIVE_SHA:-?} PLAN_HAS_R=${PLAN_HAS_R:-?} at=$(date -u +%FT%TZ)"; } > logs/run_$1.log; }
...
speed_arm(){ local NAME=$1 B=$2 EXP=$3 CA=$4; shift 4; local G=1 R=1 E=1; [ "$EXP" = 0 ] && { G=0; R=0; E=0; }
  ...
  env "$@" E4B_SERVE_EXP_INT4=$EXP E4B_SERVE_ATTN_INT4_CALIB=$CA E4B_CALIB_SOURCE=c4 E4B_FUSE_T1_GLUE=$G E4B_FUSE_T1_GLUE_R2=$R E4B_FUSE_ROUTER_EPI=$E \
    perl -e "alarm $(arm_alarm); exec @ARGV" python $W/step_decomp.py --model "$MID" --arena "$QA" --calib $W/calib.json --placement-override all-vram --amort off \
      ...
      ${K16P5_CENSUS_OUT:+--replay-profile-out "$K16P5_CENSUS_OUT"} \
...
census_arm(){ local NAME=$1 B=$2 EXP=$3; local extra=()
  ...
  K16P5_CENSUS_OUT=$W/logs/census_$NAME.txt speed_arm "$NAME" "$B" "$EXP" 0 ${extra[@]+"${extra[@]}"}
}
rc_any=0
# int4_b16: P42's arm verbatim (uncalibrated int4 attention, rows > 1 on the cached bf16 matmul).
# int4_b16_smallm: the SAME bytes with the K16 route on (E4B_ATTN_INT4_SMALLM=1). Order: control, route off, route on.
can_run 900 nf4_b16          && { K16P5_CENSUS_OUT=$W/logs/census_nf4_b16.txt          speed_arm nf4_b16 16 0 0 || rc_any=$?; }
can_run 900 int4_b16         && { K16P5_CENSUS_OUT=$W/logs/census_int4_b16.txt         speed_arm int4_b16 16 1 0 E4B_SERVE_ATTN_INT4=1 || { r=$?; [ "$rc_any" = 0 ] && rc_any=$r; }; }
can_run 900 int4_b16_smallm  && { K16P5_CENSUS_OUT=$W/logs/census_int4_b16_smallm.txt  speed_arm int4_b16_smallm 16 1 0 E4B_SERVE_ATTN_INT4=1 E4B_ATTN_INT4_SMALLM=1 || { r=$?; [ "$rc_any" = 0 ] && rc_any=$r; }; }
# proof the route engaged: the smallm arm's census must carry the K16 kernel and the int4_b16 arm must not
if [ -s "$W/logs/census_int4_b16_smallm.txt" ]; then
  n_k16=$(grep -ac "gemm_int4_b32_smallm" "$W/logs/census_int4_b16_smallm.txt"); n_k16_off=$(grep -ac "gemm_int4_b32_smallm" "$W/logs/census_int4_b16.txt" 2>/dev/null); n_k16_off=${n_k16_off:-0}
  echo "K16ROUTE smallm_arm_kernel_rows=$n_k16 route_off_arm_kernel_rows=$n_k16_off" >> summary.txt
  [ "$n_k16" -gt 0 ] || { echo "K16ROUTE NOT ENGAGED in int4_b16_smallm" >> summary.txt; [ "$rc_any" = 0 ] && rc_any=43; }
  [ "$n_k16_off" = 0 ] || { echo "K16ROUTE LEAKED into int4_b16" >> summary.txt; [ "$rc_any" = 0 ] && rc_any=44; }
fi
for n in nf4_b16 int4_b16 int4_b16_smallm; do
