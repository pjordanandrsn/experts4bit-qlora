#!/bin/bash
# P66 rehearsal, round 7 (after the rebase onto main 0.37.3): (a) P66_MODE=prove through the REAL install path at the
...
E4B=80204893083ec84057a8cd78993e55bbc361aae0; G4=f88df1ebe7944aa7481f0c784219fad879c7df89
R=/root/p66
...
env -u PYTHONPATH $BASE P66_RUN_ID=p66-rehearsal-a2000-r7-prove P66_RUN_NONCE=$NONCE P66_DEADLINE_EPOCH=$(( $(date +%s) + 600 )) P66_MODE=prove \
...
rm -rf /w/work/r7prove; mkdir -p /w/work/r7prove; cp -R $R/* /w/work/r7prove/; rm -rf /w/work/r7prove/src
echo "===== (b) full, final tree"
stage; cd $R; NONCE=$(python -c 'import secrets; print(secrets.token_hex(32))'); T0=$(date +%s)
env $COMMON P66_RUN_ID=p66-rehearsal-a2000-r7-full P66_RUN_NONCE=$NONCE P66_DEADLINE_EPOCH=$(( $(date +%s) + 5400 )) \
