#!/bin/bash
# P66 rehearsal, round 5: (a) P66_MODE=prove through the REAL install path (git + pip from GitHub at the main SHAs;
# package code identical to the branch), (b) P66_MODE=full with the final staged tree. NOT a reading.
set -u
...
E4B=94842a224f0691a008d25a509bd220e4dcb49ca7; G4=66d41c821bb29e37551a721f91b16b8ecbb49ed9
R=/root/p66
...
COMMON="P66_INSTANCE_ID=nas-a2000 E4B_SHA=$E4B GNF4_SHA=$G4 P66_REHEARSAL_CLASS=A2000 P66_REHEARSAL_MIN_VRAM_MB=8500 P66_REHEARSAL_MIN_DISK_GB=50 P66_REHEARSAL_MIN_RAM_GB=16 P66_REHEARSAL_PIN_GIB=4 P66_REHEARSAL_QWEN_SNAPSHOT=/models/OLMoE-1B-7B-0924"
echo "===== (a) prove, real install"
stage; cd $R; NONCE=$(python -c 'import secrets; print(secrets.token_hex(32))'); T0=$(date +%s)
env -u PYTHONPATH $COMMON P66_RUN_ID=p66-rehearsal-a2000-prove P66_RUN_NONCE=$NONCE P66_DEADLINE_EPOCH=$(( $(date +%s) + 600 )) P66_MODE=prove \
...
grep -v "^\s*$" outer.log | tail -25
rm -rf /w/work/r5prove; mkdir -p /w/work/r5prove; cp -R $R/* /w/work/r5prove/; rm -rf /w/work/r5prove/src
echo "===== (b) full, final tree"
stage; cd $R; NONCE=$(python -c 'import secrets; print(secrets.token_hex(32))'); T0=$(date +%s)
env $COMMON P66_RUN_ID=p66-rehearsal-a2000-full P66_RUN_NONCE=$NONCE P66_DEADLINE_EPOCH=$(( $(date +%s) + 5400 )) \
