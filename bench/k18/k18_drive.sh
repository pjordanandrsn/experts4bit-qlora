#!/bin/bash
# bench/k18/k18_drive.sh -- lane K18, CONTROLLER side: the launcher's --command. The lane's
# prereg, bench and results live in grouped-nf4-gemm (kernel/PREREG-k18-fused-splitk-gemv.md);
# only the runner is here. Stages k18_run.sh, starts it detached under a fresh nonce, polls
# TP_DONE.<nonce>, fetches receipts. Nothing here creates, destroys or approves compute.
set -uo pipefail
say(){ echo "[$(date -u +%FT%TZ)] [k18_drive] $*"; }
...
STAGE="$HERE/k18_run.sh $HERE/staged.sha256 $P60R/eids_b16.int16.bin $P60R/eids_b16.int16.json"
...
POLL=${K18_POLL_S:-60}; W=/root/k18
NONCE=$(python3 -c 'import secrets; print(secrets.token_hex(32))') || { say "refusing: no nonce"; exit 78; }
PASS="K18_RUN_ID=$RUN_ID K18_RUN_NONCE=$NONCE K18_DEADLINE_EPOCH=$DEADLINE K18_INSTANCE_ID=$E4B_RENT_INSTANCE_ID E4B_SHA=$E4B_SHA GNF4_SHA=$GNF4_SHA "
if [ "${K18_DRIVE_DRYRUN:-0}" = "1" ]; then echo "DRYRUN stage -> root@$HOST:$W ; start: env $PASS bash k18_run.sh ; poll TP_DONE.$NONCE until $DEADLINE ; fetch -> $RUN_DIR/k18"; exit 0; fi
say "run $RUN_ID nonce=$NONCE -> $HOST:$PORT; e4b $E4B_SHA (from $REPO); receipts -> $RUN_DIR/k18; deadline $DEADLINE"
$SSH "rm -rf -- $W && mkdir -p $W/logs" || { say "stage failed: remote cleanup"; exit 20; }
$SCP $STAGE "root@$HOST:$W/" && $SSH "cd $W || exit 20; nohup env $PASS bash k18_run.sh > outer.log 2>&1 < /dev/null & child=\$!; end=\$((\$(date +%s)+30)); while [ \$(date +%s) -lt \$end ]; do [ \"\$(cat K18_RUN_NONCE 2>/dev/null)\" = '$NONCE' ] && { echo started:\$child; exit 0; }; kill -0 \$child 2>/dev/null || { wait \$child; echo child-exited-early:rc=\$? >&2; exit 125; }; sleep 1; done; echo nonce-handshake-timeout >&2; exit 124" || { say "start failed: child did not bind the nonce"; exit 21; }
...
rm -rf "$RUN_DIR/k18" && mkdir -p "$RUN_DIR/k18" || { say "fetch failed: local dir"; exit 22; }
rsync -az -e "ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new -p $PORT" --exclude src --exclude 'venv*' --exclude '.cache' "root@$HOST:$W/" "$RUN_DIR/k18/" || { say "fetch failed: rsync"; exit 22; }
say "fetched $(ls "$RUN_DIR/k18" | wc -l | tr -d ' ') entries"
[ "$(cat "$RUN_DIR/k18/K18_RUN_NONCE" 2>/dev/null)" = "$NONCE" ] || { say "stale or foreign nonce in fetched artifacts"; exit 24; }
[ -f "$RUN_DIR/k18/TP_DONE.$NONCE" ] || { say "lane did not finish (no TP_DONE for this run)"; exit 23; }
RC=$(cat "$RUN_DIR/k18/K18_EXIT_CODE.$NONCE" 2>/dev/null); case "$RC" in ''|*[!0-9]*) say "malformed exit code"; exit 24;; esac
[ -f "$RUN_DIR/k18/K18_SUCCESS.$NONCE" ] && [ "$RC" = 0 ] || { say "lane exit rc=$RC without success marker"; exit "$RC"; }
