#!/bin/bash
# bench/p319/p319_drive.sh -- lane P319, CONTROLLER side: the launcher's --command.
# Stages p319_run.sh + p319_probe.py, starts the lane detached under a fresh nonce,
# polls for TP_DONE.<nonce> until the deadline, fetches the run dir. One box, no
# arguments; the arms are fixed by bench/p319/P319-PREREG.md. This lane is SHORT
...
set -uo pipefail
say(){ echo "[$(date -u +%FT%TZ)] [p319_drive] $*"; }
...
STAGE="$HERE/p319_run.sh $HERE/p319_probe.py"
...
POLL=${P319_POLL_S:-20}; W=/root/p319
NONCE=$(python3 -c 'import secrets; print(secrets.token_hex(32))') || { say "refusing: no nonce"; exit 78; }
PASS="P319_RUN_ID=$RUN_ID P319_RUN_NONCE=$NONCE P319_DEADLINE_EPOCH=$DEADLINE GNF4_BASE_SHA=$GNF4_BASE_SHA GNF4_HEAD_SHA=$GNF4_HEAD_SHA "
if [ "${P319_DRIVE_DRYRUN:-0}" = "1" ]; then
  echo "DRYRUN stage $STAGE -> root@$HOST:$W ; start: env $PASS bash p319_run.sh ; poll TP_DONE.$NONCE until $DEADLINE ; fetch -> $RUN_DIR/p319"
  exit 0
fi
say "run $RUN_ID nonce=$NONCE -> $HOST:$PORT; base $GNF4_BASE_SHA head $GNF4_HEAD_SHA; receipts -> $RUN_DIR/p319; deadline $DEADLINE"
...
$SSH "cd $W || exit 20; nohup env $PASS bash p319_run.sh > outer.log 2>&1 < /dev/null & child=\$!; end=\$((\$(date +%s)+60)); while [ \$(date +%s) -lt \$end ]; do [ \"\$(cat P319_RUN_NONCE 2>/dev/null)\" = '$NONCE' ] && { echo started:\$child; exit 0; }; kill -0 \$child 2>/dev/null || { wait \$child; echo child-exited-early:rc=\$? >&2; exit 125; }; sleep 1; done; echo nonce-handshake-timeout >&2; exit 124" \
...
while :; do
  ...
  out=$($SSH "cd $W 2>/dev/null || exit 3; if [ -f TP_DONE.$NONCE ]; then echo DONE:\$(cat P319_RC 2>/dev/null); else pgrep -f 'bash p319_run.sh' >/dev/null && echo ALIVE || echo GONE; fi; tail -1 outer.log 2>/dev/null" 2>/dev/null)
  state=$(printf '%s\n' "$out" | head -1)
  last=$(printf '%s\n' "$out" | tail -1)
  case "$state" in
...
mkdir -p "$RUN_DIR/p319"
$SCP -r "root@$HOST:$W/summary.txt" "root@$HOST:$W/logs" "root@$HOST:$W/outer.log" "$RUN_DIR/p319/" 2>/dev/null \
  || say "warning: some artefacts did not come back"
say "fetched -> $RUN_DIR/p319 (rc=$rc_wait)"
[ -s "$RUN_DIR/p319/summary.txt" ] && sed -n '1,120p' "$RUN_DIR/p319/summary.txt"
exit "$rc_wait"
