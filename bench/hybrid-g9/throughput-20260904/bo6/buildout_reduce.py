...
# (E4B_INT4_GPTQ_DAMP + streamed calibration), bo6c @ae9dc122 again (calibration determinism, the 64k pack on
# wikitext, the shipping stack on both texts). NF4 numerics do not touch #384 and do not depend on the cut.
CUT = {("qwen3", "nf4"): "f42924d", ("qwen3", "nf4_rep1"): "ae9dc12", ("qwen3", "nf4_c4val_rep1"): "ae9dc12",
       ...
       ("qwen3", "calibexp_c4val_rep2"): "ae9dc12", ("qwen3", "all_calibexp_n128"): "ae9dc12",
...
CALIBRATED = {("qwen3", "calibexp"), ("qwen3", "all_calibexp"), ("qwen3", "calibexp_c4val_rep1"),
              ...
              ("qwen3", "calibexp_c4val_rep2"), ("qwen3", "all_calibexp_n128"),
...
EXPECTED = {("qwen3", "nf4"): {"wikitext", "c4val1"}, ("qwen3", "calibexp"): {"c4val1"},
            ...
            ("qwen3", "calibexp_d01"): {"c4val1"}, ("qwen3", "calibexp_n128"): {"c4val1", "wikitext"},
            ("qwen3", "calibexp_n512"): {"c4val1"}, ("qwen3", "calibexp_c4val_rep2"): {"c4val1"},
            ("qwen3", "all_calibexp_n128"): {"wikitext", "c4val1"},
            ("mixtral", "nf4"): {"wikitext", "c4val1"}, ("mixtral", "lic_calibexp"): {"wikitext", "c4val1", "b1", "b16"},
            ("mixtral", "lic_calibexp_n128"): {"c4val1"}}
K8_DET = "the K8 instrument is run-to-run deterministic on one box + cut (bake included)"
CAL_DET = "streamed GPTQ calibration (Hessian tap, GPU solve, min_rows fallbacks) + packing + K8 are run-to-run deterministic on one box + cut"
REPEATS = {"qwen3": [("wikitext", "nf4", ["nf4_rep1"], K8_DET), ("c4val1", "nf4", ["nf4_c4val_rep1", "nf4_c4val_rep2"], K8_DET),
                     ("c4val1", "calibexp_c4val_rep1", ["calibexp_c4val_rep2"], CAL_DET)]}
SPEED_NOTE = {"qwen3": "- `all_calibexp_n128` (the licensed stack): speed NOT on this lane -- bo7 measures the calibrated stack at "
                       "B=1 / B=16 with the hook's 16k-token default (`E4B_CALIB_NSEQ` unset), not the 64k pack scored here."}
...
DESC = {("qwen3", "nf4"): "NF4 experts, bf16 attention -- re-scored on this box and cut (baseline, both texts)",
        ...
        ("qwen3", "calibexp_n128"): "calibrated int4 experts alone, 4x the calibration set (E4B_CALIB_NSEQ=128): c4val1 from bo6b's sweep, wikitext from bo6c",
        ("qwen3", "calibexp_c4val_rep2"): "exact repeat of `calibexp_c4val_rep1` after a re-bake (bo6c: calibration determinism)",
        ("qwen3", "all_calibexp_n128"): "THE SHIPPING STACK: streamed 64k-token calibrated int4 experts + C4-calibrated int4 attention (192 projections, the same 32 batches) + r1 + r2 (rope-only fold) + router epilogue + #385 glue (bo6c, both texts)",
...
def verdict(fam, cfg, deltas, is_base, pending, dnats=None, floor=None):
    """deltas: {text: dppl}. Returns (per-text cells, registered verdict) in ppl units. A licensed improvement whose
    delta on a text is inside the family's arithmetic-order floor is called parity there, never an improvement by a number."""
    if is_base:
    ...
    if cal:
        if n >= 2 and all(dp < 0 for dp in deltas.values()):
            s = f"pass on {n} texts — LICENSED as registered (same sign on both)"
            sub = [t for t in deltas if floor is not None and dnats and abs(dnats.get(t, 0.0)) <= floor]
            if sub:
                s += f"; sub-floor on {', '.join(sub)} → at parity there, no improvement claimed by a number"
            return cells, s
        if n >= 2:
...
def main():
    ...
    for fam in FAMS:
        ...
        print("|---|---|---|---|---|---|---|---|---|---|---|---|---|---|")
        order = ORDER[fam] + sorted(c for c in fams if c not in ORDER[fam] and not any(c in reps for _, _, reps, _ in REPEATS.get(fam, [])))
        for cfg in order:
            r = fams.get(cfg, {"ppl": {}})
            ...
            nll_s, d_s, deltas, dnats, pending, dead, meth = {}, {}, {}, {}, set(), set(), None
            for t in TEXTS:
                p = r["ppl"].get(t)
                if not p:
                    d_s[t] = "—"
            ...
            cells, ver = verdict(fam, cfg, deltas, is_base, pending, dnats, floor)
        ...
        if reps:
            print(f"\nRepeat controls ({NAME[fam]}; each phase re-baked the released checkpoint to NF4 -- bo6b after attempt 3 freed the arena, bo6c after bo6b -- and re-scored on the same box and window; `mean_nll` compared at full float precision):")
            print("| text | original arm | nll | repeats | nll | spread (nats) | reading |")
            print("|---|---|---|---|---|---|---|")
            for t, orig, rl, det in reps:
                ...
                reading = (f"bit-identical → {det}" if spread == 0 and same
        ...
        if sp:
            ...
            for c, r in sp:
                ...
                print(s)
            if fam in SPEED_NOTE:
                print(SPEED_NOTE[fam])
