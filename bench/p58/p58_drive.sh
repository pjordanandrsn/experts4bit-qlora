#!/bin/bash
# bench/p58/p58_drive.sh -- lane P58, CONTROLLER side: the launcher's --command. Stages P39's harness pieces,
# P42's hook (P54's), P37's vLLM driver (byte-identical), the reducer and p58_run.sh; starts the lane detached
...
set -uo pipefail
say(){ echo "[$(date -u +%FT%TZ)] [p58_drive] $*"; }
...
P39="$REPO/bench/p39"; P37="$REPO/bench/h2h-20260905/p37"
STAGE="$HERE/p58_run.sh $HERE/p58_reduce.py $P37/p37_vllm.py $P39/step_decomp.py $P39/k8_bake.py $P39/calib.json $HERE/staged.sha256"
...
while read -r want name; do
  ...
  case "$name" in p58_run.sh|p58_reduce.py) src="$HERE/$name";; p37_vllm.py) src="$P37/$name";; hook/*) src="$REPO/bench/p42/$name";; *) src="$P39/$name";; esac
  got=$(sha_of "$src")
...
POLL=${P58_POLL_S:-60}; W=/root/p58
NONCE=$(python3 -c 'import secrets; print(secrets.token_hex(32))') || { say "refusing: no nonce"; exit 78; }
PASS="P58_RUN_ID=$RUN_ID P58_RUN_NONCE=$NONCE P58_DEADLINE_EPOCH=$DEADLINE P58_INSTANCE_ID=$E4B_RENT_INSTANCE_ID E4B_SHA=$E4B_SHA GNF4_SHA=$GNF4_SHA ${P58_VLLM_PRIMARY:+P58_VLLM_PRIMARY=$P58_VLLM_PRIMARY }${P58_VLLM_SECONDARY:+P58_VLLM_SECONDARY=$P58_VLLM_SECONDARY }"
if [ "${P58_DRIVE_DRYRUN:-0}" = "1" ]; then echo "DRYRUN stage -> root@$HOST:$W ; start: env $PASS bash p58_run.sh ; poll TP_DONE.$NONCE until $DEADLINE ; fetch -> $RUN_DIR/p58"; exit 0; fi
say "run $RUN_ID nonce=$NONCE -> $HOST:$PORT; e4b $E4B_SHA (from $REPO); receipts -> $RUN_DIR/p58; deadline $DEADLINE"
...
$SSH "cd $W || exit 20; nohup env $PASS bash p58_run.sh > outer.log 2>&1 < /dev/null & child=\$!; end=\$((\$(date +%s)+30)); while [ \$(date +%s) -lt \$end ]; do [ \"\$(cat P58_RUN_NONCE 2>/dev/null)\" = '$NONCE' ] && { echo started:\$child; exit 0; }; kill -0 \$child 2>/dev/null || { wait \$child; echo child-exited-early:rc=\$? >&2; exit 125; }; sleep 1; done; echo nonce-handshake-timeout >&2; exit 124" || { say "start failed: child did not bind the nonce"; exit 21; }
...
# summary line, (b) GPU util, (c) workdir size and free disk, (d) whether `bash p58_run.sh` is still there.
...
STALL_S=${P58_STALL_S:-900}; P58_MIN_PROGRESS_MB=${P58_MIN_PROGRESS_MB:-16}
progress_verdict() {  # idle_s stall_s util dfk_now dfk_prev du_now du_prev -> "" | "fetching:<delta>" | "stall:<idle_s>"
  idle_s=$1; stall_s=$2; util=$3; dfk_now=$4; dfk_prev=$5; du_now=$6; du_prev=$7; min_mb=$P58_MIN_PROGRESS_MB
  consumed=0; grew=0
...
while :; do
  ...
  hb=$($SSH "echo \"\$(grep -v '^[[:space:]]*$' $W/summary.txt 2>/dev/null | tail -n 1 | cut -c1-160) | gpu \$(nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits 2>/dev/null | tr -d ' ') | du \$(du -sm $W 2>/dev/null | cut -f1)M | disk \$(df -h /root | tail -1 | awk '{print \$4}') | dfk \$(df -k /root | tail -1 | awk '{print \$4}') | live \$(pgrep -f 'bash p58_run.sh' | wc -l | tr -d ' ')\"" 2>/dev/null)
  ...
  if [ -n "$(lane_dead "$live" "$LAST_LIVE")" ]; then
    say "LANE DEAD: no 'bash p58_run.sh' on the box for two consecutive polls and no TP_DONE -- the remote process exited without writing its markers; not waiting out the deadline"
    LANE_DEAD=1; break
  fi
  LAST_LIVE=$live
...
rm -rf "$RUN_DIR/p58" && mkdir -p "$RUN_DIR/p58" || { say "fetch failed: local dir"; exit 22; }
...
rsync -az -e "ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new -p $PORT" --exclude 'artifact*/payloads/layer_*' --include 'work_qwen3/' --include 'work_qwen3/bake.json' --exclude 'work_qwen3/*' --exclude 'venv*' --exclude '.cache' "root@$HOST:$W/" "$RUN_DIR/p58/" || { say "fetch failed: rsync"; exit 22; }
say "fetched $(ls "$RUN_DIR/p58" | wc -l | tr -d ' ') entries"
[ "$(cat "$RUN_DIR/p58/P58_RUN_NONCE" 2>/dev/null)" = "$NONCE" ] || { say "stale or foreign nonce in fetched artifacts"; exit 24; }
[ -f "$RUN_DIR/p58/TP_DONE.$NONCE" ] || {
  # A lane the box killed is a different fact from a lane that ran out of clock (e4b#634/#641).
  [ "${LANE_DEAD:-0}" = 1 ] && { say "lane DIED on the box: its process was gone with no P58_EXIT_CODE and no TP_DONE -- artifacts fetched, nothing measured"; exit 25; }
  say "lane did not finish (no TP_DONE for this run)"; exit 23; }
RC=$(cat "$RUN_DIR/p58/P58_EXIT_CODE.$NONCE" 2>/dev/null); case "$RC" in ''|*[!0-9]*) say "malformed exit code"; exit 24;; esac
[ -f "$RUN_DIR/p58/P58_SUCCESS.$NONCE" ] && [ "$RC" = 0 ] || { say "lane exit rc=$RC without success marker"; exit "$RC"; }
