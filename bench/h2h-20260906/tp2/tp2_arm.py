...
HARNESS = "tp2_arm.py (p38_arm.py as amended + T1-T9)"
...
def load_e4b(a):
    ...
    if a.attn_4bit:
        try:
            x["n_attn4"] = quantize_attention_projections_4bit(model)     # U4: BEFORE the attention LoRA (its docstring's rule)
        ...
        if x["n_layers"] and x["n_attn4"] != 4 * x["n_layers"]:
            stub(a, "void_attn4", f"quantize_attention_projections_4bit converted {x['n_attn4']} projections, expected {4 * x['n_layers']}",
                 {"phase": "attn4", "n_layers": x["n_layers"], "model_type": x["model_type"]}, code=3)
...
def main():
    ...
    ap.add_argument("--selftest", action="store_true", help="T8: CPU, tiny synthetic model, both branches, mocked kernels")
