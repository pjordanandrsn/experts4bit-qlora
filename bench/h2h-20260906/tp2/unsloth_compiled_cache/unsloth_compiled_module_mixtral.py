...
try:
    ...
    pass
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.mixtral.modeling_mixtral import (F, Callable, Optional, torch, nn, init, ACT2FN, Cache, use_kernel_func_from_hub, FlashAttentionKwargs, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, Unpack, TransformersKwargs, maybe_autocast, MixtralConfig)
...
@torch_compile_with_fallback(fullgraph = True, dynamic = True, options = torch_compile_options)
def MixtralTopKRouter_forward(self, hidden_states):
    hidden_states = hidden_states.reshape(-1, self.hidden_dim)
    router_logits = F.linear(hidden_states, self.weight)  # (seq_len, num_experts)
    router_logits = torch.nn.functional.softmax(router_logits.float(), dim=-1, dtype = torch.float32).to(router_logits.float().dtype)
...
class MixtralTopKRouter(nn.Module):
    ...
    def forward(self, hidden_states):
        return MixtralTopKRouter_forward(self, hidden_states=hidden_states)


@torch_compile_with_fallback(fullgraph = False, dynamic = True, options = torch_compile_options)
def MixtralSparseMoeBlock_forward(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        return MixtralSparseMoeBlock_forward(self, hidden_states=hidden_states)


@torch_compile_with_fallback(fullgraph = True, dynamic = True, options = torch_compile_options)
def MixtralRMSNorm_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class MixtralRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return MixtralRMSNorm_forward(self, hidden_states=hidden_states)
...
def MixtralRotaryEmbedding_forward(self, x, position_ids):
...
class MixtralRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return MixtralRotaryEmbedding_forward(self, x=x, position_ids=position_ids)
...
class MixtralAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return MixtralAttention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask, past_key_values=past_key_values, **kwargs)
