...
@torch_compile_with_fallback(fullgraph = False, dynamic = True, options = torch_compile_options)
def Gemma4AudioAttention_forward(
    self,
    ...
    attn_weights = F.softmax(attn_weights, dim=-1, dtype = torch.float32).to(attn_weights.dtype).to(value_states.dtype)
...
class Gemma4AudioAttention(nn.Module):
    ...
    def forward(
        ...
        return Gemma4AudioAttention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask)
...
@torch.compiler.disable(recursive = False)
def Gemma4TextAttention_forward(self, *args, **kwargs):
