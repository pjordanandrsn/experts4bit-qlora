...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.olmoe.modeling_olmoe import (F, Callable, Optional, torch, nn, init, ACT2FN, Cache, use_kernel_func_from_hub, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, Unpack, TransformersKwargs, maybe_autocast, OlmoeConfig, OlmoeExperts)

@torch_compile_with_fallback(fullgraph = True, dynamic = True, options = torch_compile_options)
def OlmoeRMSNorm_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class OlmoeRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return OlmoeRMSNorm_forward(self, hidden_states=hidden_states)
...
def OlmoeRotaryEmbedding_forward(self, x, position_ids):
...
class OlmoeRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return OlmoeRotaryEmbedding_forward(self, x=x, position_ids=position_ids)


@torch_compile_with_fallback(fullgraph = False, dynamic = True, options = torch_compile_options)
def OlmoeMLP_forward(self, x):
...
class OlmoeMLP(nn.Module):
    ...
    def forward(self, x):
        return OlmoeMLP_forward(self, x=x)
...
class OlmoeAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return OlmoeAttention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask, past_key_values=past_key_values, **kwargs)


@torch_compile_with_fallback(fullgraph = True, dynamic = True, options = torch_compile_options)
def OlmoeTopKRouter_forward(self, hidden_states):
...
class OlmoeTopKRouter(nn.Module):
    ...
    def forward(self, hidden_states):
        return OlmoeTopKRouter_forward(self, hidden_states=hidden_states)


@torch_compile_with_fallback(fullgraph = False, dynamic = True, options = torch_compile_options)
def OlmoeSparseMoeBlock_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class OlmoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return OlmoeSparseMoeBlock_forward(self, hidden_states=hidden_states)
