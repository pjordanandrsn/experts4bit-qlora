...
try:
    ...
    pass
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.qwen3_moe.modeling_qwen3_moe import (F, Callable, Optional, torch, nn, init, ACT2FN, Cache, use_kernel_func_from_hub, FlashAttentionKwargs, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, Unpack, TransformersKwargs, maybe_autocast, Qwen3MoeConfig)
...
class Qwen3MoeAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return Qwen3MoeAttention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask, past_key_values=past_key_values, **kwargs)
...
class Qwen3MoeMLP(nn.Module):
    ...
    def forward(self, x):
        return Qwen3MoeMLP_forward(self, x=x)
...
@torch_compile_with_fallback(fullgraph = True, dynamic = True, options = torch_compile_options)
def Qwen3MoeTopKRouter_forward(self, hidden_states):
...
class Qwen3MoeTopKRouter(nn.Module):
    ...
    def forward(self, hidden_states):
        return Qwen3MoeTopKRouter_forward(self, hidden_states=hidden_states)
...
@torch_compile_with_fallback(fullgraph = True, dynamic = True, options = torch_compile_options)
def Qwen3MoeRMSNorm_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class Qwen3MoeRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return Qwen3MoeRMSNorm_forward(self, hidden_states=hidden_states)
...
class Qwen3MoeRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return Qwen3MoeRotaryEmbedding_forward(self, x=x, position_ids=position_ids)
