...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.granitemoe.modeling_granitemoe import (F, Callable, Optional, torch, nn, init, ACT2FN, Cache, use_kernel_func_from_hub, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, Unpack, TransformersKwargs, maybe_autocast, GraniteMoeConfig, GraniteMoeParallelExperts, GraniteMoeTopKGating)

@torch_compile_with_fallback(fullgraph = True, dynamic = True, options = torch_compile_options)
def GraniteMoeRMSNorm_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class GraniteMoeRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return GraniteMoeRMSNorm_forward(self, hidden_states=hidden_states)
...
def GraniteMoeRotaryEmbedding_forward(self, x, position_ids):
...
class GraniteMoeRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return GraniteMoeRotaryEmbedding_forward(self, x=x, position_ids=position_ids)


@torch.compiler.disable(recursive = False)
def GraniteMoeParallelExperts_forward(self, inputs, expert_size):
    """
...
class GraniteMoeParallelExperts(nn.Module):
    ...
    def forward(self, inputs, expert_size):
        return GraniteMoeParallelExperts_forward(self, inputs=inputs, expert_size=expert_size)


@torch_compile_with_fallback(fullgraph = False, dynamic = True, options = torch_compile_options)
def GraniteMoeMoE_forward(self, layer_input):
...
class GraniteMoeMoE(nn.Module):
    ...
    def forward(self, layer_input):
        return GraniteMoeMoE_forward(self, layer_input=layer_input)
...
@torch.compiler.disable(recursive = False)
def GraniteMoeAttention_forward(
    self,
...
class GraniteMoeAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return GraniteMoeAttention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask, past_key_values=past_key_values, **kwargs)
