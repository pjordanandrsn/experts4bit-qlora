...
## Amendment 1 (2026-09-21, registered AFTER the run, before any replacement measurement)

Recorded here as well as in `RESULTS-p54.md` so a reader of the pre-registration sees what it could not deliver.

**P4 was unbuildable as written.** `--series-out` requires `--amort on` (`step_decomp.py` refuses the flag otherwise), and `--amort on` is refused by the captured B>1 stage: `AssertionError: amort-armed runs keep the baseline dispatch path (not capturable)`. So an untimed `--amort on` arm at B=16 with the graph loop cannot exist. The lane exited on its own `SERIES MISSING` guard (rc 45) and nothing was measured for P4.

**Replacement, registered:** the distinct-expert count is read either from the **b1d stage** with `--amort on` (that stage takes it) on batch-16-shaped prompts, or from `--profile-out`'s per-expert `tokens_routed` histogram, which `step_decomp` writes under the same `--amort on` requirement without the captured loop. Whichever is used, **that arm's step time is not quoted** — per-layer counters and event syncs perturb it. It rides the next lane that already rents this model rather than a box of its own.

**Gate on any B=16 default (P5's follow-up), registered:** one arm with fused projections and the round-2 glue forced to the unfused chain (to separate the kernel-shape hypothesis from the glue-path one), plus a KL-from-checkpoint read (`bench/p44/kl_serve.py`) of the fused stack against the control. The bar is the one already on `main` for shipped configurations: **≤ 0.10 nats AND top-1 ≥ 0.93**. No B=16 default before that passes.

**The rule this lane bought:** an untimed diagnostic arm is dry-run against the harness's own argument checks — on CPU, for free — **before** its pre-registration merges. This is the second lane in three (after P52's Gemma-4 K8 gate) where an instrument was registered that could not be built.
