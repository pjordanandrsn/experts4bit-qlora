#!/bin/bash
# bench/p57/p57_drive.sh -- lane P57, CONTROLLER side: the launcher's --command. Stages P39's harness pieces,
# P57's hook (P42's plus the distinct-expert counter), the instrument, and p57_run.sh; starts the lane detached
...
P39="$REPO/bench/p39"
STAGE="$HERE/p57_run.sh $HERE/distinct_experts.py $P39/step_decomp.py $P39/k8_bake.py $P39/calib.json $HERE/staged.sha256"
...
while read -r want name; do
  ...
  case "$name" in p57_run.sh|distinct_experts.py) src="$HERE/$name";; hook/*) src="$HERE/$name";; *) src="$P39/$name";; esac
  got=$(sha_of "$src")
...
$SSH "cd $W || exit 20; nohup env $PASS bash p57_run.sh > outer.log 2>&1 < /dev/null & child=\$!; end=\$((\$(date +%s)+30)); while [ \$(date +%s) -lt \$end ]; do [ \"\$(cat P57_RUN_NONCE 2>/dev/null)\" = '$NONCE' ] && { echo started:\$child; exit 0; }; kill -0 \$child 2>/dev/null || { wait \$child; echo child-exited-early:rc=\$? >&2; exit 125; }; sleep 1; done; echo nonce-handshake-timeout >&2; exit 124" || { say "start failed: child did not bind the nonce"; exit 21; }
...
# summary line, (b) GPU util, (c) workdir size and free disk, (d) whether `bash p57_run.sh` is still there.
...
STALL_S=${P57_STALL_S:-900}; P57_MIN_PROGRESS_MB=${P57_MIN_PROGRESS_MB:-16}
progress_verdict() {  # idle_s stall_s util dfk_now dfk_prev du_now du_prev -> "" | "fetching:<delta>" | "stall:<idle_s>"
  idle_s=$1; stall_s=$2; util=$3; dfk_now=$4; dfk_prev=$5; du_now=$6; du_prev=$7; min_mb=$P57_MIN_PROGRESS_MB
  consumed=0; grew=0
...
while :; do
  ...
  hb=$($SSH "echo \"\$(grep -v '^[[:space:]]*$' $W/summary.txt 2>/dev/null | tail -n 1 | cut -c1-160) | gpu \$(nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader,nounits 2>/dev/null | tr -d ' ') | du \$(du -sm $W 2>/dev/null | cut -f1)M | disk \$(df -h /root | tail -1 | awk '{print \$4}') | dfk \$(df -k /root | tail -1 | awk '{print \$4}') | live \$(pgrep -f 'bash p57_run.sh' | wc -l | tr -d ' ')\"" 2>/dev/null)
  ...
  if [ -n "$(lane_dead "$live" "$LAST_LIVE")" ]; then
    say "LANE DEAD: no 'bash p57_run.sh' on the box for two consecutive polls and no TP_DONE -- the remote process exited without writing its markers; not waiting out the deadline"
    LANE_DEAD=1; break
  fi
  LAST_LIVE=$live
...
rm -rf "$RUN_DIR/p57" && mkdir -p "$RUN_DIR/p57" || { say "fetch failed: local dir"; exit 22; }
rsync -az -e "ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new -p $PORT" --exclude 'artifact*/payloads/layer_*' --exclude work_qwen3 --exclude 'venv*' --exclude '.cache' "root@$HOST:$W/" "$RUN_DIR/p57/" || { say "fetch failed: rsync"; exit 22; }
...
[ -f "$RUN_DIR/p57/TP_DONE.$NONCE" ] || {
  ...
  [ "${LANE_DEAD:-0}" = 1 ] && { say "lane DIED on the box: its process was gone with no P57_EXIT_CODE and no TP_DONE -- artifacts fetched, nothing measured"; exit 25; }
  say "lane did not finish (no TP_DONE for this run)"; exit 23; }
RC=$(cat "$RUN_DIR/p57/P57_EXIT_CODE.$NONCE" 2>/dev/null); case "$RC" in ''|*[!0-9]*) say "malformed exit code"; exit 24;; esac
[ -f "$RUN_DIR/p57/P57_SUCCESS.$NONCE" ] && [ "$RC" = 0 ] || { say "lane exit rc=$RC without success marker"; exit "$RC"; }
