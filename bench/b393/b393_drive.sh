#!/bin/bash
# bench/b393/b393_drive.sh -- lane B393, CONTROLLER side: the launcher's --command. The lane's
# prereg, census and results live in grouped-nf4-gemm (kernel/PREREG-b393-combine-reduce-bitwise.md,
# kernel/b393_bitwise_census.py); only the runner is here. Pattern: bench/b374/b374_drive.sh. Stages b393_run.sh, starts it detached under a fresh nonce, polls
# TP_DONE.<nonce>, fetches receipts. Nothing here creates, destroys or approves compute.
set -uo pipefail
say(){ echo "[$(date -u +%FT%TZ)] [b393_drive] $*"; }
...
# Only the runner is staged. The census script itself is cloned ON THE BOX from
# grouped-nf4-gemm at GNF4_SHA, because pod-launch pins adertha and e4b only.
STAGE="$HERE/b393_run.sh $HERE/staged.sha256"
...
POLL=${B393_POLL_S:-60}; W=/root/b393
NONCE=$(python3 -c 'import secrets; print(secrets.token_hex(32))') || { say "refusing: no nonce"; exit 78; }
PASS="B393_RUN_ID=$RUN_ID B393_RUN_NONCE=$NONCE B393_DEADLINE_EPOCH=$DEADLINE B393_INSTANCE_ID=$E4B_RENT_INSTANCE_ID E4B_SHA=$E4B_SHA GNF4_SHA=$GNF4_SHA "
if [ "${B393_DRIVE_DRYRUN:-0}" = "1" ]; then echo "DRYRUN stage -> root@$HOST:$W ; start: env $PASS bash b393_run.sh ; poll TP_DONE.$NONCE until $DEADLINE ; fetch -> $RUN_DIR/b393"; exit 0; fi
say "run $RUN_ID nonce=$NONCE -> $HOST:$PORT; e4b $E4B_SHA (from $REPO); receipts -> $RUN_DIR/b393; deadline $DEADLINE"
$SSH "rm -rf -- $W && mkdir -p $W/logs" || { say "stage failed: remote cleanup"; exit 20; }
$SCP $STAGE "root@$HOST:$W/" && $SSH "cd $W || exit 20; nohup env $PASS bash b393_run.sh > outer.log 2>&1 < /dev/null & child=\$!; end=\$((\$(date +%s)+30)); while [ \$(date +%s) -lt \$end ]; do [ \"\$(cat B393_RUN_NONCE 2>/dev/null)\" = '$NONCE' ] && { echo started:\$child; exit 0; }; kill -0 \$child 2>/dev/null || { wait \$child; echo child-exited-early:rc=\$? >&2; exit 125; }; sleep 1; done; echo nonce-handshake-timeout >&2; exit 124" || { say "start failed: child did not bind the nonce"; exit 21; }
...
rm -rf "$RUN_DIR/b393" && mkdir -p "$RUN_DIR/b393" || { say "fetch failed: local dir"; exit 22; }
rsync -az -e "ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new -p $PORT" --exclude src --exclude 'venv*' --exclude '.cache' "root@$HOST:$W/" "$RUN_DIR/b393/" || { say "fetch failed: rsync"; exit 22; }
say "fetched $(ls "$RUN_DIR/b393" | wc -l | tr -d ' ') entries"
[ "$(cat "$RUN_DIR/b393/B393_RUN_NONCE" 2>/dev/null)" = "$NONCE" ] || { say "stale or foreign nonce in fetched artifacts"; exit 24; }
[ -f "$RUN_DIR/b393/TP_DONE.$NONCE" ] || { say "lane did not finish (no TP_DONE for this run)"; exit 23; }
RC=$(cat "$RUN_DIR/b393/B393_EXIT_CODE.$NONCE" 2>/dev/null); case "$RC" in ''|*[!0-9]*) say "malformed exit code"; exit 24;; esac
[ -f "$RUN_DIR/b393/B393_SUCCESS.$NONCE" ] && [ "$RC" = 0 ] || { say "lane exit rc=$RC without success marker"; exit "$RC"; }
