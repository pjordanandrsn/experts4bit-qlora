...
# P45 (bench/p45/P45-PREREG.md): where a training step's time goes. Two arms at the field recipe, each with 3 warm + 3 profiled
# steps (torch.profiler CPU+CUDA in tp4_arm.py --profile-steps) and a 1 s nvidia-smi dmon sampler (sm%, mem%, PCIe rx/tx MB/s)
# running beside the arm. e4b fused_attn4 (T1's arm, 29.3 s/step at 16 % util) and the Unsloth comparator on the same box.
...
prof_family(){ local FAM=$1 MID=$2 REV=$3 FAL=$4 AL=${TP4_PROF_ALARM:-$5} PS=${TP4_PROFILE_STEPS:-3} PW=${TP4_PROFILE_WARM:-3}
  ...
  FETCH_REASON=""; fetch $FAM $MID $REV $FAL; local frc=$?
  if [ $frc -ne 0 ]; then local st=not_run; [ $frc -eq 2 ] && st=load_fault
    for t in "e4b fused_attn4 fused" "unsloth ckpt_unsloth unsloth"; do set -- $t; stubw $FAM $1 $2 $3 $st "$FETCH_REASON"; done
    free_family $FAM ${MID//\//--}; return 0; fi
  local TOK=$W/tokens_$FAM.json
  if ! tokenise $FAM "$MID" $REV alpaca $SEQ $W/data/ds_alpaca.json $DS_ALPACA_SHA $TOK; then
    local why; why="tokenise failed (logs/prepare_${FAM}_alpaca.log): $(tail -1 logs/prepare_${FAM}_alpaca.log | cut -c1-200)"
    for t in "e4b fused_attn4 fused" "unsloth ckpt_unsloth unsloth"; do set -- $t; stubw $FAM $1 $2 $3 harness_error "$why"; done
    free_family $FAM ${MID//\//--}; return 0; fi
  local TS; TS=$(tok_sha $TOK); echo "TOKENS $FAM alpaca sha=$TS" | tee -a summary.txt
